...
# Split the combined string into its base64 encoded components
IFS='#' read -r ProjNameEncoded ReceiversEncoded msgEncoded <<< "$combined"

ProjName=$(echo "$ProjNameEncoded" | base64 -d)
Receivers=$(echo "$ReceiversEncoded" | base64 -d)
msg=$(echo "$msgEncoded" | base64 -d)

function send_message(){
    [[ $Receivers == null ]] && return

    CompanyId=$(cat ./workspace/hzisemt/etc/wechat.json | ./workspace/hzisemt/libs/jq -r '.CompanyID')
    AgentId=$(cat ./workspace/hzisemt/etc/wechat.json | ./workspace/hzisemt/libs/jq -r '.AgentID')
    Secret=$(cat ./workspace/hzisemt/etc/wechat.json | ./workspace/hzisemt/libs/jq -r '.Secret')

    response=$(curl -s -G "https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid=$CompanyId&corpsecret=$Secret")
    
    # 从response中提取access_token
    access_token=$(echo $response | ./workspace/hzisemt/libs/jq -r '.access_token')
...
EOF
    ...
    # 调用发送消息的API
    curl -s -d "$data" -H "Content-Type: application/json" -X POST "https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=$access_token"  > /dev/null 
}
...
Receivers=$(cat etc/sys_config.json | ../libs/jq -r '.msg_receivers')
AlarmReceivers=$(cat etc/sys_config.json | ../libs/jq -r '.alarm_receivers')
LeaderReceivers=$(cat etc/sys_config.json | ../libs/jq -r '.leader_receivers')  # 新增的leader接收组
Relay=$(cat etc/sys_config.json | ../libs/jq -r '.via_relay')
HomeSSH=$(cat etc/sys_config.json | ../libs/jq -r '.home_ssh')
if [[ $HomeSSH == null ]]; then
    HomeSSH=2022
fi
...
function send_message(){
    ...
    elif [[ $group == "alarm" ]]; then
        groupReceivers=$AlarmReceivers
    elif [[ $group == "leader" ]]; then
        groupReceivers=$LeaderReceivers  # 处理leader接收组
    else
        echo "未知的消息接收组: $group"
        return
    fi

    [[ $groupReceivers == null ]] && return
    
    ProjName=$(cat etc/sys_config.json | ../libs/jq -r '.project_name')
    if [[ $ProjName == null ]]; then
        ProjName="项目名称未知: "
    else
        ProjName="$ProjName: "
    fi
    ...
    ProjNameEncoded=$(echo -n "$ProjName" | base64)
    ReceiversEncoded=$(echo -n "$groupReceivers" | base64)
    msgEncoded=$(echo -n "$msg" | base64 -w 0)
    combined="$ProjNameEncoded#$ReceiversEncoded#$msgEncoded"

    if [[ $Relay == null || $Relay == "false" ]]; then
        nc -z -w5 218.23.107.89 $HomeSSH &&
        ssh -p $HomeSSH 218.23.107.89 "./workspace/hzisemt/remote_msg.sh '$combined'"
    else
        nc -z -w5 localhost $HomeSSH &&
        ssh -p $HomeSSH localhost "./workspace/hzisemt/remote_msg.sh '$combined'"
    fi
}
...
# 测试：
# send_message "msg" "运维消息测试"
# send_message "alarm" "警报消息测试"
# send_message "leader" "领导消息测试"  # 新增的leader消息测试

# wk：
# send_message "msg" "用户组"
# send_message "alarm" "运维组"
# send_message "leader" "领导消息测试"  # 新增的leader消息测试
```
...
if (hzi::config.warningTimeSync) {
          ...
          std::thread timeSync(syncWarningInfo);//微震事件预警
          timeSync.detach();
        }
```
```cpp
/***
flag true 查询周六 8-10点
flag false 查询8-10点
*/
bool isSaturdayMorning(int64_t timestamp,bool flag) {
    // 将毫秒时间戳转换为秒，因为 std::chrono::system_clock 以秒为单位处理
    std::chrono::milliseconds ms(timestamp);
    std::chrono::system_clock::time_point timePoint(ms);

    // 将 time_point 转换为 std::time_t，以便我们可以使用 std::localtime
    ...
    // 判断是否在上午 8 点到 10 点之间
    if (localTime->tm_hour >= 10 && localTime->tm_hour < 12) {
        return true;
    }
...
void syncWarningInfo(){
  //  uint64_t tmp = 1723219200000;
    while(1){
         std::this_thread::sleep_for(
            std::chrono::minutes(hzi::config.warningTimeSyncIntval));
        auto ms_now = nowMS();
     ...
     //   tmp += 2*60*60*1000;
        ...
        auto flag = getWarningMsg(ms_now);
        if(flag){
            // cout<<" 得到预警信息："<<std::to_string(ms_now)<<endl;
            std::stringstream ss;
            ss<<" 得到预警信息："<<std::to_string(ms_now)<<endl;
            logInfo(ss.str());
        }else{
            if(isSaturdayMorning(ms_now,true) ){
                //是否周六 8--10点
                if(!isSaturday){
                    //查询 是否有发送信息
                    checkWarninghMsg(ms_now);
                }
                isSaturday = true;
            }else{
                isSaturday = false;
...
```cpp
//handleWarning.hpp
#ifndef HANDLEWARNING_HPP
#define HANDLEWARNING_HPP
...
struct WarningInfoParm{
    ...
    float effectPro;
    float basicValue;
    uint64_t lastSampTime;
    uint64_t basicSampTime;
    uint64_t spFromTime;
    uint64_t spEndTime;
  };
/***
 * 定时查询信息，微震和自电数据 半个小时查一次？视电阻率可以根据计算视电阻率时进行判断
 * 返回是否预警
*/
tl::expected<bool, string> getWarningMsg(uint64_t ms_now);
tl::expected<bool, string> doWarningMsgProcess(uint64_t fromsampTime, uint64_t endsampTime, bool isRecalc=false);
bool checkWarninghMsg(uint64_t ms_now);
void checkProName(string &proName);
tl::expected<bool, string> sendKafkaMsevtsMsg(uint64_t fromsampTime, uint64_t endsampTime);
#endif
```
```cpp
//handleWarning.cpp
#include "handleWarning.hpp"
#include "kafka_producer.hpp"

/**
 * 获取微震等级
 */
int getMsWarningLevel(vector<int> &LevelVec)
{
    ...
    {
        // cout<<" i = "<<i<<", "<<LevelVec[i]<<endl;
        if (LevelVec[i] == 3)
        {
            R++;
        }
        else if (LevelVec[i] == 2)
        {
            O++;
        }
        else if (LevelVec[i] == 1)
        {
            Y++;
        }
    }
 ...
 * 读取摘要文件，
 */
void readEngDenity(uint64_t fromsampTime, uint64_t endsampTime,
                   int channels, int classId, int Width, std::map<int, std::vector<float>> &chnMap)
{
    uint64_t midnite = midnight(endsampTime);
    uint64_t endSampTime = endsampTime - midnite;                                    // 当日相对采样时刻(ms)
    uint64_t fromSampTime = (fromsampTime > midnite) ? (fromsampTime - midnite) : 0; // 当日相对采样时刻(ms) 暂时不考虑跨日
    double endtime = ((double)endSampTime) / 1000.0;
    double fromtime = ((double)fromSampTime) / 1000.0;
    if (endtime - fromtime <= 0)
    {
        return;
    }

    uint16_t du_end = std::floor(endtime * Width / 3600);
    uint16_t du_from = std::floor(fromtime * Width / 3600);

    uint32_t pixelWidth = 24 * Width;
    auto size1 = channels * pixelWidth * 4;
    auto size2 = channels * pixelWidth * 8;
    auto size3 = channels * pixelWidth * 16;
    string digest = "digest_" + std::to_string(Width) + ".dat"; //"digest_4.dat"
...
{
    auto conn = hzi::config.pConnPool->getConnection();
    string sqlStr = "SELECT cust_name FROM `e_info` ";
    auto rslt = conn.executeQuery(sqlStr.c_str());
    if (rslt.next())
    {
        proName = rslt.getString("cust_name");
    }
}
bool checkWarninghMsg(uint64_t ms_now)
{
    uint64_t constNum = 7 * 24 * 60 * 60 * 1000; // 7D
    // uint64_t midnite = midnight(ms_now); //当天0点时间戳
    auto curMS = ms_now - constNum;
    auto conn = hzi::config.pConnPool->getConnection();
    string sqlStr = "SELECT * FROM `e_warning_Info` WHERE tm > " + std::to_string(curMS) + "  LIMIT 1";
    auto rslt = conn.executeQuery(sqlStr.c_str());
    if (rslt.next())
    {
        // 有记录
    ...
    {
        string msg = "渗流电场:正常;视电阻率:稳定;本周数据无异常;";
        string cmd("./send_message.sh \"msg\" \"" + msg + "\"");
        system(cmd.c_str());

        string cmd2 = "./send_message.sh \"alarm\" \"" + msg + "\"";
        system(cmd2.c_str());
        return true;
    }
}

tl::expected<bool, string> doWarningMsgProcess(uint64_t fromsampTime, uint64_t endsampTime, bool isRecalc)
{
    try
    {
        /*
         ...
         * highEnergyNum 大能量事件；
         * msLocation 定位 ；
         * e_sp_variation 自电变化量;
         * ee_p_variation 视电阻率变化量
         */
        bool isMS = true;
        bool isEE = true;
        int leveL_sp = 0;
        ...
        WarningInfoParm parm;
        auto rslt_parm = conn.executeQuery("SELECT * FROM `e_warningInfo_parm`");
        if (rslt_parm.next())
        {
            parm.thresholdEnergy = rslt_parm.getDouble("thresholdEnergy"); // 大能量
            parm.roofZ = rslt_parm.getDouble("roofZ");                     // 微震频次
            parm.floorZ = rslt_parm.getDouble("floorZ");
            parm.electrodes = rslt_parm.getInt("electrodes");          // 用到
            parm.sp_basicValue = rslt_parm.getDouble("sp_basicValue"); // 自电基础值
            parm.basicValue = rslt_parm.getDouble("basicValue");       // 温度基础值
            parm.effectPro = rslt_parm.getDouble("effectPro");
            parm.lastSampTime = rslt_parm.getLLong("lastSampTime");
            parm.basicSampTime = rslt_parm.getLLong("basicSampTime");
            parm.spFromTime = rslt_parm.getLLong("spFromTime");
            parm.spEndTime = rslt_parm.getLLong("spEndTime");
        }
        else
        {
            // cout << "e_warningInfo_parm 为空，请补全参数" << endl;
            std::stringstream ss;
            ss<<"e_warningInfo_parm 为空，请补全参数";
            logErr(ss.str());
            return tl::make_unexpected(string("e_warningInfo_parm 为空，请补全参数"));
        }

        if (isMS)
        {
            std::map<string, vector<float>> warningLevelMapMS;
            auto rslt_level = conn.executeQuery("SELECT * FROM `e_warning_level_value`");

            while (rslt_level.next())
            {
                string item = rslt_level.getString("item");
                auto valueY = rslt_level.getDouble("valueY");
                auto valueO = rslt_level.getDouble("valueO");
                auto valueR = rslt_level.getDouble("valueR");
                vector<float> levelMS;
                levelMS.push_back(valueY);
                levelMS.push_back(valueO);
                levelMS.push_back(valueR);
                warningLevelMapMS[item] = levelMS;
            }

            if (warningLevelMapMS.find("msFrequency") == warningLevelMapMS.end() ||
                warningLevelMapMS.find("highEnergyNum") == warningLevelMapMS.end() ||
                warningLevelMapMS.find("msLocation") == warningLevelMapMS.end())
            {
                // cout << "e_warning_level_value 有误，请补全地震相关参数" << endl;
                std::stringstream ss;
                ss << "e_warning_level_value 有误，请补全地震相关参数";
                logErr(ss.str());
                return tl::make_unexpected(string("e_warning_level_value 参数有误，请补全地震相关参数"));
            }
            ...
            vector<float> vec_y;
            vector<float> vec_z;
            vector<float> msLocation_z_top;
            // vector<float> msLocation_z_bom;
            //  vector<float> eng_z;
            ...
            while (rslt_ms.next())
            {
                ...
                // 判定微震频次
                msFrequency++;
                auto samp_time = rslt_ms.getDouble("samp_time");
                auto loc_x = rslt_ms.getDouble("loc_x");
                auto loc_y = rslt_ms.getDouble("loc_y");
                auto loc_z = rslt_ms.getDouble("loc_z");
                auto energy = rslt_ms.getDouble("energy");
                auto magnitude = rslt_ms.getDouble("magnitude");
                sum_eng += energy;

                // 判定是否是大能量
                if (energy > parm.thresholdEnergy)
                {
                    highEnergyNum++;
                    vec_x.push_back(loc_x);
                    vec_y.push_back(loc_y);
                    vec_z.push_back(loc_z);
                    // eng_z.push_back(loc_z);
                }

                // 微震 异常区域个数
                if (loc_z > parm.roofZ) //|| loc_z < parm.floorZ
                {
                    msLocation++;
                    if (loc_z > parm.roofZ)
                        msLocation_z_top.push_back(loc_z);
                    // if(loc_z < parm.floorZ)msLocation_z_bom.push_back(loc_z);
                }
            }
            if (rowCount == 0)
            {
                // cout << "e_msevt_rslts 无记录！" << endl;
                std::stringstream ss;
                ss << "e_msevt_rslts 无记录！";
                logErr(ss.str());
                return tl::make_unexpected(string("e_msevt_rslts 为空，请补全参数！"));
            }

            cout << "msFrequency = " << msFrequency << endl;
            // if (msFrequency < 2) return ret;

            /*判断微震等级*/
            // 微震频次
            if (msFrequency >= warningLevelMapMS["msFrequency"][0] && warningLevelMapMS["msFrequency"][0] != 0)
            {
                msg += "微震：该时间范围内,总频次为 " + std::to_string(msFrequency) + ",总能量为" + std::to_string(sum_eng) + "J";
            }

            std::vector<int> msLevelVec(3, 0);
            cout << " msLocation = " << msLocation << " ,highEnergyNum = " << highEnergyNum << ",msFrequency = " << msFrequency << endl;
            if (msFrequency >= warningLevelMapMS["msFrequency"][2] && warningLevelMapMS["msFrequency"][2] != 0)
            {
                msLevelVec[0] = 3;

                // msg+="微震频次 "+std::to_string(msFrequency)+" 达到红色预警值;";
            }
            else if (msFrequency >= warningLevelMapMS["msFrequency"][1] && warningLevelMapMS["msFrequency"][1] != 0)
            {
                msLevelVec[0] = 2;
                // msg+="频次 "+std::to_string(msFrequency)+" 达到橙色预警值;";
            }
            else if (msFrequency >= warningLevelMapMS["msFrequency"][0] && warningLevelMapMS["msFrequency"][0] != 0)
            {
                msLevelVec[0] = 1;
                //  msg+="微震频次 "+std::to_string(msFrequency)+" 达到蓝色预警值;";
            }

            // 微震 大能量个数
            if (highEnergyNum >= warningLevelMapMS["highEnergyNum"][0] && warningLevelMapMS["highEnergyNum"][0] != 0)
            {
                // 找到 x 坐标的最小值和最大值
                ...
                msg += "大能量事件 " + std::to_string(highEnergyNum) + " 次且主要分布在 ";
                string msg_fw = "范围：" + std::to_string(min_x) + "<x<" + std::to_string(max_x) + "," + std::to_string(min_y) + "<y<" + std::to_string(max_y) + "," + std::to_string(min_z) + "<z<" + std::to_string(max_z) + ";";
                msg += msg_fw;
            }

            if (highEnergyNum >= warningLevelMapMS["highEnergyNum"][2] && warningLevelMapMS["highEnergyNum"][2] != 0)
            {
                msLevelVec[1] = 3;
                // msg+="微震大能量事件个数 "+std::to_string(highEnergyNum)+" 达到红色预警值;";
            }
            else if (highEnergyNum >= warningLevelMapMS["highEnergyNum"][1] && warningLevelMapMS["highEnergyNum"][1] != 0)
            {
                msLevelVec[1] = 2;
                // msg+="微震大能量事件个数 "+std::to_string(highEnergyNum)+" 达到橙色预警值;";
            }
            else if (highEnergyNum >= warningLevelMapMS["highEnergyNum"][0] && warningLevelMapMS["highEnergyNum"][0] != 0)
            {
                msLevelVec[1] = 1;
                //  msg+="微震大能量事件个数 "+std::to_string(highEnergyNum)+" 达到蓝色预警值;";
            }

            // 微震 异常区域个数
            if (msLocation >= warningLevelMapMS["msLocation"][0] && warningLevelMapMS["msLocation"][0] != 0)
            {
                msg += "异常区域事件个数 " + std::to_string(msLocation) + " 高程范围 ";
                auto min_z = std::min_element(msLocation_z_top.begin(), msLocation_z_top.end());
                auto max_z = std::max_element(msLocation_z_top.begin(), msLocation_z_top.end());
                string msg_fw = " " + std::to_string(*min_z) + "~" + std::to_string(*max_z) + ";";
            }
            if (msLocation >= warningLevelMapMS["msLocation"][2] && warningLevelMapMS["msLocation"][2] != 0)
            {
                msLevelVec[2] = 3;
                //  msg+="微震异常区域事件个数 "+std::to_string(msLocation)+" 达到红色预警值;";
            }
            else if (msLocation >= warningLevelMapMS["msLocation"][1] && warningLevelMapMS["msLocation"][1] != 0)
            {
                msLevelVec[2] = 2;
                //  msg+="微震异常区域事件个数 "+std::to_string(msLocation)+" 达到橙色预警值;";
            }
            else if (msLocation >= warningLevelMapMS["msLocation"][0] && warningLevelMapMS["msLocation"][0] != 0)
            {
                msLevelVec[2] = 1;
                //  msg+="微震异常区域事件个数 "+std::to_string(msLocation)+" 达到蓝色预警值;";
            }
            msLevel = getMsWarningLevel(msLevelVec);

            if (msLevel > 0)
            {
                msg += (msLevel == 1) ? "微震预警蓝色" : "微震预警橙色";
            }

            // cout<<" msLevel = "<<msLevel<<","<<msLevelVec[0]<<","<<msLevelVec[1]<<","<<msLevelVec[2]<<endl;
            warningType = msLevel;
        }
        ...
        {
            // 电法参数逻辑
            std::map<string, vector<float>> warningLevelMapEE;
            auto rslt_level = conn.executeQuery("SELECT * FROM `e_warning_level_value`");
            ...
            while (rslt_level.next())
            {
                ...
                auto valueO = rslt_level.getDouble("valueO");
                auto valueR = rslt_level.getDouble("valueR");
                vector<float> levelEE;
                levelEE.push_back(valueY);
                levelEE.push_back(valueO);
                levelEE.push_back(valueR);
                warningLevelMapEE[item] = levelEE;
            }

            if (warningLevelMapEE.find("e_sp_variation") == warningLevelMapEE.end() || warningLevelMapEE.find("ee_p_variation") == warningLevelMapEE.end())
            {
                cout << "e_warning_level_value 有误，请补全电法相关参数" << endl;
                return tl::make_unexpected(string("e_warning_level_value 参数有误，请补全电法相关参数"));
            }

            int channels = hzi::reChannelsLabels.size();
            std::map<int, std::vector<float>> chnMap;
            for (int i = 0; i < channels; i++)
            {
                std::vector<float> pmatchlist;
                chnMap[i + 1] = pmatchlist;
            }
            int classId = 0;
            int Width = 16;  // 15min 2^4
            int Width2 = 16; // 2min 2^5

            readEngDenity(fromsampTime, endsampTime, channels, classId, Width, chnMap);

            std::map<int, std::vector<float>> chnBasicMap;
            for (int i = 0; i < channels; i++)
            {
                std::vector<float> pmatchlist;
                chnBasicMap[i + 1] = pmatchlist;
            }
            readEngDenity(parm.spFromTime, parm.spEndTime, channels, classId, Width2, chnBasicMap);

            std::map<int, float> chnBasic;
            for (int i = 0; i < channels; i++)
            {

                float sum = 0.0f;
                for (float value : chnBasicMap[i + 1])
                {
                    sum += value;
                }
                auto base = sum / chnBasicMap[i + 1].size();
                chnBasic[i + 1] = base;
                //  cout<<" i = "<<i+1<<" , "<<base<<endl;
            }
            int dataLen = chnMap[1].size();
            std::vector<int> reNumList;
            auto threValueVec = warningLevelMapEE["e_sp_variation"];
            int maxCount = 0;
            ...
            for (int k = 0; k < dataLen; k++)
            {
                ...
                for (int i = 0; i < channels; i++)
                {
                    if (chnMap[i + 1][k] - chnBasic[i + 1] >= threValueVec[0])
                        yellowCount++;
                    if (chnMap[i + 1][k] - chnBasic[i + 1] >= threValueVec[1])
                        orangeCount++;
                    if (chnMap[i + 1][k] - chnBasic[i + 1] >= threValueVec[2])
                        redCount++;
                }
                // 根据计数确定预警级别
                if (redCount >= 1 && orangeCount >= parm.electrodes)
                {
                    temp = 3;
                }
                else if (orangeCount >= parm.electrodes)
                {
                    temp = 2;
                }
                else if (yellowCount >= parm.electrodes)
                {
                    temp = 1;
                }
                leveL_sp == (temp > leveL_sp) ? temp : leveL_sp;
                maxCount = (yellowCount > maxCount) ? yellowCount : maxCount;
            }
            if (maxCount == hzi::reChannelsLabels.size())
            {
                msg += "提醒检查N极,并重置基础值！";
            }
            else if (leveL_sp > 0)
            {
                if (leveL_sp == 3)
                {
                    //  msg += "红色预警：预警电极个数达到 "+ std::to_string(maxCount)+"个;";
                    warningType = leveL_sp;
                }
                else if (leveL_sp == 2)
                {
                    //   msg += "橙色预警：预警电极个数达到 "+ std::to_string(maxCount)+"个;";
                    warningType = leveL_sp;
                }
                else if (leveL_sp == 1)
                {
                    //  msg += "蓝色预警：预警电极个数达到 "+ std::to_string(maxCount)+"个;";
                    warningType = (warningType > leveL_sp) ? warningType : (warningType + leveL_sp);
                }
            }
            msg += "渗流电场:正常;视电阻率:稳定;综合预警：蓝色;";
        }

        // 发送预警信息到企业微信
        if (leveL_sp > 0 || msLevel > 0)
        {
            // 给运维(msg)发，让运维在页面处理
            string cmd2 = "./send_message.sh \"msg\" \"" + msg + "\"";
            system(cmd2.c_str());

            if (isRecalc)
            {
                // 重算，若无该记录则插入，若有则更新，新记录均：未审核未发送状态，重新进入运维审核。
                auto prep = conn.prepareStatement(
                    "INSERT INTO e_warning_Info "
                    "(tm, warningType, ms_warning, ee_warning, msg, is_handle, is_sent_user, suggestions) "
                    "values (?, ?, ?, ?, ?, DEFAULT, DEFAULT, DEFAULT) ON DUPLICATE KEY UPDATE  "
                    "warningType = values(warningType),ms_warning = values(ms_warning),"
                    "ee_warning = values(ee_warning),msg = values(msg),"
                    "is_handle = DEFAULT,is_sent_user = DEFAULT,suggestions = DEFAULT");
                conn.beginTransaction();
                prep.bind(1, (double)fromsampTime);
                prep.bind(2, warningType);
                prep.bind(3, msLevel);
                prep.bind(4, leveL_sp);
                prep.bind(5, msg);
                prep.execute();
            ...
            {
                // 自动，若无记录则插入（未审核未发送状态），若有则更新（保持审核发送状态同更新前）
                auto prep = conn.prepareStatement(
                    "INSERT INTO e_warning_Info "
                    "(tm, warningType, ms_warning, ee_warning, msg, is_handle, is_sent_user, suggestions) "
                    "values (?, ?, ?, ?, ?, DEFAULT, DEFAULT, DEFAULT) ON DUPLICATE KEY UPDATE  "
                    "warningType = values(warningType),ms_warning = values(ms_warning),"
                    "ee_warning = values(ee_warning),msg = values(msg),"
                    "is_handle = values(is_handle),is_sent_user = values(is_sent_user),suggestions = values(suggestions)");
                conn.beginTransaction();
                prep.bind(1, (double)fromsampTime);
                prep.bind(2, warningType);
                prep.bind(3, msLevel);
        ...
        // 给用户组(alarm)发送蓝色和处理后的非蓝色，同时运维组也发一份(告知已发送给用户)
        // string sqlStr = "SELECT warningType, msg FROM `e_warning_Info` WHERE is_handle=1 ORDER BY tm DESC LIMIT 1"; // 取最新的一条
        string sqlQuery = "SELECT tm, `warningType` AS wt, `msg` FROM e_warning_Info WHERE `warningType`=1 AND `is_sent_user`=0 \
                        UNION SELECT tm, `warningType` AS wt, `msg` FROM e_warning_Info WHERE `warningType`> 1 AND `is_handle`= 1 AND `is_sent_user`= 0 ";
        auto msgSentToUsers = conn.executeQuery(sqlQuery.c_str());
        while (msgSentToUsers.next())
        {
            uint64_t tm = msgSentToUsers.getDouble("tm");
            string msgTmp = msgSentToUsers.getString("msg");
            string cmdAlarm = "./send_message.sh \"alarm\" \"" + msgTmp + "\"";                               // 发用户
            string cmdMsg("./send_message.sh \"msg\" \"" + string(" 【预警已发送给用户】 ") + msgTmp + "\""); // 发运维

            int flag = !system(cmdMsg.c_str());
            flag = !system(cmdAlarm.c_str()) && flag;
            ...
            else
            {
                ...
                ss<<"企业微信发送失败..";
                logErr(ss.str());
                return tl::make_unexpected("企业微信发送失败");
            }
        }
        return true;
    }
...
tl::expected<bool, string> getWarningMsg(uint64_t ms_now)
{
    try
    {
        uint64_t constNum = 2 * 60 * 60 * 1000; // 2h时间
        uint64_t midnite = midnight(ms_now);    // 当天0点时间戳
        auto curMS = ms_now - midnite;
        int index = curMS / constNum;     // 0 - 11
        int leftValue = curMS % constNum; // 0 - 2*60*60*1000-1
        int constTimerLen = 15 * 60 * 1000;
        uint64_t fromsampTime;
        uint64_t endsampTime;
        if (leftValue < constTimerLen)
        {
            // 处理上一个2h数据
            fromsampTime = midnite + (index - 1) * constNum;
            endsampTime = midnite + index * constNum - 1;
        }
        else
        {
            fromsampTime = midnite + index * constNum;
            endsampTime = midnite + (index + 1) * constNum - 1;
        }

        return doWarningMsgProcess(fromsampTime, endsampTime);
    }
    catch (const sql_exception &e)
    {
        std::cerr << e.what() << '\n';
    ...
    try
    {
        if (true)
        {
            ...
            while (rslt_ms.next())
            {
                ...
                doc.AddMember("samp_time", rapidjson::Value(samp_time), alloc);
                doc.AddMember("loc_x", loc_x, alloc);
                doc.AddMember("loc_y", loc_y, alloc);
                doc.AddMember("loc_z", loc_z, alloc);
                doc.AddMember("energy", energy, alloc);
                doc.AddMember("magnitude", magnitude, alloc);
                ...
                // 发送到 Kafka
                if (KafkaProducer::kafkaProducer)
                {
                    if (!KafkaProducer::kafkaProducer->sendMessage(hzi::config.kafkaTopicMicroseismic, json_str))
                    {
                        std::cerr << "Failed to send microseismic data to Kafka: " << json_str << std::endl;
                    }
                    else
                    {
                        std::cout << "Sent microseismic data to Kafka: " << json_str << std::endl;
                    }
                }
            }
            if (rowCount == 0)
            {
                cout << "e_msevt_rslts 无记录！" << endl;
                return tl::make_unexpected(string("e_msevt_rslts 为空"));
            }
...
// Define a struct for the cost function
struct FittingCostFunctor {
  // Constructor to initialize the observed data point
  FittingCostFunctor(double x, double y) : x_(x), y_(y) {}

  // The operator() method that computes the residual for a data point
  template <typename T>
  bool operator()(const T *const a, const T *const b, const T *const c,
                  T *residual) const {
    // 拟合能量-源检距函数
    // y = a * x^(-b) * exp(-c * x)
    residual[0] = T(y_) - a[0] * pow(T(x_), -b[0]) * exp(-c[0] * T(x_));
    return true;
  }
...
string timeStr(std::chrono::system_clock::time_point timepnt,
               const char *fmt = "%y%m%d %H:%M:%S");

uint64_t nowMS();
time_t midnight_(uint64_t timestamp);
...
auto findMinValuePair(Container const &x) -> typename Container::value_type {
  using value_t = typename Container::value_type;
  const auto compare = [](value_t const &p1, value_t const &p2) {
    return p1.second < p2.second;
  };
  return *std::min_element(x.begin(), x.end(), compare);
}

#pragma region Generic

double dist(const LocF &loc1, const LocF &loc2);
int safePow(int base, unsigned int power);
void splitStr(const std::string &origStr, char deli, std::vector<string> &vals);
std::vector<int> parseString(const std::string &input);
//日期转换 yyyy-MM-dd hh:mm:ss 转成yyyyMMddhhmmss
std::string convertDateFormat(const std::string &date_str);
// 格式化函数：将双精度数值格式化为保留两位小数的字符串
std::string formatDouble(double value, int fmat = 2);

// timestamp: ms
std::unordered_map<string, ChanelsConfig>
getChnConfigByTime(uint8_t type_id, uint64_t timestamp);

#pragma endregion

std::tuple<double, double, double>
msEnergyFit(std::vector<std::pair<double, double>> &data);
...
// 字典 p_voltages，电压单位 mV
static std::map<uint8_t, uint8_t> voltageMap = {
    {2, 24}, {3, 48}, {4, 72}, {5, 96}};
// 字典 p_excit_types
static std::map<uint8_t, string> excitTypeMap = {
    {0, "单正"}, {1, "正负正"}, {2, "M序列"}, {3, "正负正+M序列"}};

/**
 * @brief 计算激发类型中 1 个周期的均值
 *
 * @param p         二进制数据（需要乘以对应的电流系数、电压系数）
 * @param from      本周期对应的内存开始位置
 * @param to        本周期对应的内存结束位置
 * @return double   解编结果（需要乘以对应的电流系数、电压系数）
 * @note  周期：单正是 3 个周期，正负正是 7 个周期
 */
...
// 时间串转换函数
string timeStr(std::chrono::system_clock::time_point timepnt, const char *fmt) {
  std::time_t btime = std::chrono::system_clock::to_time_t(timepnt);
  ...
  strftime(timebuffer, 80, fmt, timeinfo);
...
//timestamp：单位ms
fs::path relLocFromTime(uint64_t timestamp) {
  int64_t milli = timestamp + (int64_t)8 * 60 * 60 * 1000; // localtime
...
std::string to_string_with_precision(double val, int fix) {
  std::ostringstream out_str;
  out_str.precision(fix);
  out_str.setf(std::ios::fixed);
  // setiosflags
  out_str << val;
  return out_str.str();
}
...
std::vector<int> parseString(const std::string &input) {
  ...
  while (std::getline(ss, segment, ',')) {
    std::stringstream range_ss(segment);
    std::string range_part;

    while (std::getline(range_ss, range_part, '-')) {
      int start, end;
      std::stringstream(range_part) >> start;
      if (std::getline(range_ss, range_part, '-')) {
        std::stringstream(range_part) >> end;
...
msEnergyFit(std::vector<std::pair<double, double>> &data) {
  ...
  double a = 1000.0, b = 0.7, c = 0.002;

  // Create a Ceres problem
  ceres::Problem problem;

  // Add a residual block for each data point
  for (const auto &data_point : data) {
    ceres::CostFunction *cost_function =
        new ceres::AutoDiffCostFunction<FittingCostFunctor, 1, 1, 1, 1>(
            new FittingCostFunctor(data_point.first, data_point.second));
  ...
  // Output the results
  // std::cout << "Final a, b, c: " << a << ", " << b << ", " << c << std::endl;
...
getChnConfigByTime(uint8_t type_id, uint64_t timestamp) {
  auto conn = hzi::config.pConnPool->getConnection();
  std::unordered_map<string, ChanelsConfig> chns;
  string sql =
      "SELECT "
      "sh1.dev_id,sh1.chn_no,sh1.state_id,sh1.chn_label,sh1.loc_x,sh1.loc_y,"
      "sh1.loc_z,sh1.updatetime "
      "FROM e_chns_config_history sh1 "
      "JOIN(SELECT chn_label, MAX(updatetime) AS last_update_time FROM "
      "e_chns_config_history WHERE UNIX_TIMESTAMP(updatetime) <";
  sql += std::to_string(timestamp / 1000);
  sql += " GROUP BY chn_label) sh2 ON sh1.chn_label = sh2.chn_label AND "
         "sh1.updatetime ="
         "sh2.last_update_time where sh1.type_id=";
  sql += std::to_string(type_id);

  ResultSet rslt = conn.executeQuery(sql.c_str());
  while (rslt.next()) {
    auto label = rslt.getString("chn_label");
    auto x = rslt.getDouble("loc_x");
    auto y = rslt.getDouble("loc_y");
    auto z = rslt.getDouble("loc_z");
    auto stateId = rslt.getInt("state_id");
    auto chnNo = rslt.getInt("chn_no");
    auto devId = rslt.getInt("dev_id");
    tm updatetime = rslt.getDateTime("updatetime");
    char buffer[100]; // buffer to hold the formatted string
    std::strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", &updatetime);
    chns[label] =
        ChanelsConfig{(uint8_t)chnNo,   type_id,        (float)x,
                      (float)y,         (float)z,       label,
                      (uint8_t)stateId, (uint8_t)devId, string(buffer)};
  }

  return chns;
}

// Function to convert a timestamp in milliseconds to a formatted string
std::string timestampToString(int64_t timestamp_ms) {
  ...
  // Manually format the time as "YYYY-MM-DD HH:MM:SS.mmm"
  char buffer[24];
...
public:
  ...
  DataFrame::loadFrmFromFile(uint8_t classId, const fs::path &fileName) {
    try {
      ...
      size_t dataSize =
          df.channels() * df.points() * (df.classId() == 4 ? 2 : 4);
  ...
  */
  static tl::expected<DataFrame, string> make(uint32_t cmdSeq,
    ...
    try {
      ...
      *(uint16_t *)(pHead_ + 10) = chns;//怎么通道和点数又赋值一次？构造函数已经做过
      *(uint32_t *)(pHead_ + 12) = pnts;
      ...
      int64_t tmStmp =
          *(uint8_t *)(ptr + 21)
              ...
              : be64toh(*(uint64_t *)ptr); // 对时模块时间或采样时间

      *(double *)(pHead_) = (double)tmStmp;

      auto classId = *(uint8_t *)(pHead_ + 8) =
          *(uint8_t *)(ptr + 8); // 原始帧数据类型赋值给数据帧类型
      ...
      *(float *)(pHead_ + 19) =
          (classId == 11) ? 1000.0
                          : StartMonitor::getIntvl(classId); // 采样间隔，伽马数据写死1s，
...
#else
      // 激发类型改由基站标识
      *(uint8_t *)(pHead_ + 23) = *(uint8_t *)(ptr + 30); // 激发类型id（激励电法发射模式，0x00:单正 0x01:正负正 0x02:M序列）
#endif
      *(uint8_t *)(pHead_ + 24) = *(uint8_t *)(ptr + 19); // 激发电压id
      *(uint8_t *)(pHead_ + 25) = *(uint8_t *)(ptr + 20); // 是否正在发射

      *(uint8_t *)(pHead_ + 26) = (uint8_t)devAddr;//设备地址也是拼接掩码？
      ...
      *(uint8_t *)(pHead_ + 28) = StartMonitor::getEEvoltageSchm();
      ...
      if (classId == 4) {
        ...
        int32_t *up32 = (int32_t *)(upData.get() + 48);//数据区
...
tl::expected<int, string> DataFrame::saveIntoDataDigest() const {
  try {
    if ((hzi::config.ms_mining_type > 0) || classId() == 6 || classId() == 4 ||
      ...
      return 0;//全时微震6，温度4，伽马11，激电1不保存摘要
    ...
    for (int i = 0; i < channels(); i++) {
      ...
      //         continue;
      // } else
      if (i > cLabels.size() - 1) {//所有基站拼接的通道label数量和通道数不一致
        print("in saveIntoDataDigest问题帧");
        break;
      }
      if (hzi::chnState[cLabels[i]] != 0) {//通道损坏或耦合不佳，state_id=1/2
        continue;
      }

      //找到当前通道：点绝对值最大其值，点最大值，点最小值，点绝对值最大其索引，最大值点索引，最小值点索引
      auto label = cLabels[i];
      int64_t sum = 0;
      int init_value = *(p32 + i * points());//通道的第一个点值
      ...
      if (isFullTimeFrame()) {//inDigestDruation为true，则isFullTimeFrame()必为true
        std::unique_lock<std::mutex> lck(spMtx);
        if (inDigestDruation) {
          if (digests.contains(label)) {//不同基站的通道label会有相同的吗？
...
tl::expected<int, string> DataFrame::saveFrmToFile() const {
  ...
  try {
    ...
    ofs.write(pHead_, 32);
    if (classId() == 4) {
      ofs.write((char *)pData_, channels() * points() * 2);//温度
    } else {
...
tl::expected<uint32_t, std::string> sendCmd(uint8_t addr, uint8_t cmdId,
                                            const char *data);
...
MsEvtCnf msEvtCnf;
ViewSpace viewSpace;
std::vector<DensityColor> densityColors;
std::vector<DensityGray> densityGrays;
std::map<string, uint8_t> chnSate;
// std::map<string, std::tuple<float, float, float>> chnLocs;
}; // namespace hzi

namespace sender {
bool b_StartSync = false;
bool b_enable = false;

tl::expected<int, string> relayConfig(uint8_t cmdId, uint8_t devId, void *buf,
                                      size_t len) {
  return Client::make(hzi::config.sampler_ip, hzi::config.relayPort)//当前主机作为client连接采集主机
      .and_then([&](Client client) {
        auto buf1 = std::make_unique<char[]>(len + 10);
        for (uint8_t i = 0; i < 4; i++) {
          *(buf1.get() + i) = (uint8_t)0x99;//帧头：99999999
        }
        *(buf1.get() + 4) = (uint8_t)cmdId;
        *(buf1.get() + 5) = (uint8_t)devId;
        for (uint8_t i = 0; i < 4; i++) {
          *(buf1.get() + 6 + len + i) = (uint8_t)0x66;//帧尾：66666666
        }
        memmove(buf1.get() + 6, (char *)buf, len);
        char reply[200];
        return client.write(buf1.get(), len + 10).and_then([&](auto ret) {//发送中继帧给采集主机
          return client.read(reply, 200).and_then([&](auto bytes) {//接收采集主机回复
            reply[bytes] = '\0';//实际接收字节数
            string msg(reply);
            cout << msg << "\n";
            return tl::expected<int, string>(0);
          });
      ...
      .or_else([&](auto err) -> tl::expected<int, string> {
        return tl::make_unexpected(string("中继置参出错：") + err);
      });
}
...
void checkAllDevices() {
  while (b_enable) {
    ...
    for (auto &devicemap : hzi::config.devicesMap) {
      if (devicemap.second.devLinking) {
        auto now = system_clock::now();
        auto ms = duration_cast<milliseconds>(now.time_since_epoch());
        auto localtime = (uint64_t)ms.count();
        int t = localtime - devicemap.second.lastSampTime;
        devicemap.second.devLinking = t > 60000 ? false : true;
        if (!devicemap.second.devLinking) {
          hzi::config.deviceConnections--;
          cout << "基站： " << std::to_string(devicemap.first) << "  失联！"
               << endl;
        }
      }
    }
...
void setFrameHead(void *ptr) {
  *(uint32_t *)ptr = 0x69686968;
  return;
}

void setFrameTail(void *ptr) {
  *(uint32_t *)ptr = 0x1616;
  return;
}

tl::expected<uint32_t, string> writeSendLog(uint8_t addr, uint8_t cmdId) {
  try {
    if (cmdId == 7 || cmdId == 8 || cmdId == 9) {
      return 0;//连接检测帧，对时帧，不记录日志
    }

    Connection conn = hzi::config.pConnPool->getConnection();
    conn.execute("INSERT INTO e_send_frm_log (addr, cmd_id) values (?, ?)",
                 addr, cmdId);
    ResultSet result =
    ...
    if (result.next()) {
      return result.getInt("maxsq");
    } else {
      // cout << "db error\n";
      return tl::make_unexpected("writeSendLog error: cmd_seq not found");
    }
  } catch (sql_exception &e) {
    cout << "db exception:" << e.what() << endl;
    return tl::make_unexpected(string("writeSendLog error: ") + e.what());
  }
}

//将数据区data根据cmdId等拼成完整原始帧
tl::expected<int, string> setSendBuf(char *buf, uint8_t addr, uint8_t cmdId,
                                     uint32_t cmdSeq, const char *data) {
  try {
    auto p = buf;
    setFrameHead(p);
    p += 4;
    uint32_t followBytes = hzi::config.sendFrmBytes[cmdId] - 8;
    *(uint32_t *)p = htobe32(followBytes); // 后续字节数
    p += 4;
    *p = 1; // 校验标志 0x0f
    p++;
    *p = 15; // 设备标识
    ...
    *p = cmdId; // 命令字
    p++;
    *(uint32_t *)p = htobe32(cmdSeq); //  命令标识
    p += 4;
    size_t dLen = hzi::config.sendFrmBytes[cmdId] - 22;//数据区长度

    if (dLen > 0) {
      memcpy(p, data, dLen); // 数据区
    }
    ...
    return tl::make_unexpected(string("setSendBuf error: ") + e.what());
  }
}
 ...
 * @return cmdSeq
*/
tl::expected<uint32_t, string> sendCmd(uint8_t addr, uint8_t cmdId,
                                       const char *data) {
  size_t bytes = hzi::config.sendFrmBytes[cmdId];
  auto pBuf = std::make_unique<char[]>(bytes);
  uint32_t cmdSeq;
  //   if (hzi::config.devSockMap[addr] == 0) {
  //     return tl::make_unexpected("sendCmd error for cmdId:" +
  //     to_string(cmdId) +
  //                                " to device address:" + to_string(addr) +
  //                                ": not linked");
  //   }

  return writeSendLog(addr, cmdId)
      .and_then([&](uint32_t seq) {
        if (cmdId == 4) //|| cmdId == 8 || cmdId == 9
        {
          cmdSeq = seq - addr + 1;//启动监测
        } else {
          cmdSeq = seq;
        }
        if (cmdId == 3) {//采集置参
          auto type_id =
              *(uint8_t *)(data + 176); // 激电激发类型 0-单正 1-正负正 2-序列
          auto pw_time_id = *(uint8_t *)(data + 177); // 激电供电时间id
          //  auto voltage_id =  *(uint8_t*)(data + 178); //激电发射电压id
          auto m_order_id = *(uint8_t *)(data + 179); // M序列阶数ID
          auto m_wid_id = *(uint8_t *)(data + 180);   // M序列码元宽度ID
          auto times_id = *(uint8_t *)(data + 181); // 激电定时发射次数
          // auto samp_pnts = *(uint16_t*)(data + 230);
          // auto samp_intvl_id =  *(uint8_t*)(data + 232);
          auto conn2 = hzi::config.pConnPool->getConnection();

          auto rslt0 = conn2.executeQuery(
              "select time_win from p_time_wins where win_id = ? ", pw_time_id);
          rslt0.next();

          auto pw_time = rslt0.getInt("time_win"); // 激电供电时间

          // auto rslt1 = conn2.executeQuery(
          //     "SELECT voltage FROM p_voltages WHERE vol_id = ? "
          //     ,voltage_id);
          // rslt1.next();
          // auto voltage = rslt1.getInt("voltage");   //激电发射电压

          // 激电通道数

          auto rslt2 = conn2.executeQuery(
              "SELECT COUNT(*) from e_chns_config where  type_id= 2 and "
              "dev_id < ?  ",
              addr);
          rslt2.next();
          auto chnns = rslt2.getInt("COUNT(*)"); // 通道数

          auto addTimeValue = 0;
          std::map<int, int> timesMap;
          timesMap.insert_or_assign(0, 0);
          timesMap.insert_or_assign(1, 4);
          timesMap.insert_or_assign(2, 8);
          timesMap.insert_or_assign(3, 12);

          if (type_id == 0) {
            addTimeValue = pw_time * 3 * chnns; // ms 转 s
            // addTimeValue = pw_time *3* chnns*(addr-1);  //ms 转 s
            addTimeValue =
                addTimeValue > 0 ? (int)((addTimeValue + 500) / 1000 + 30) : 0;

          } else if (type_id == 1) {
            addTimeValue = pw_time * 7 * chnns;
            addTimeValue =
                addTimeValue > 0 ? (int)((addTimeValue + 500) / 1000 + 30) : 0;

          } else {
            std::map<int, int> orderMap;
            orderMap.insert_or_assign(0, 6);
            orderMap.insert_or_assign(1, 7);
            orderMap.insert_or_assign(2, 8);

            auto m_order = orderMap[m_order_id]; // M序列阶数

            auto rsltM = conn2.executeQuery(
                "select time_win from p_time_wins where win_id = ? ", m_wid_id);
            rsltM.next();
            auto m_wid = rsltM.getInt("time_win"); // M序列码元宽度

            if (type_id == 2) {
              addTimeValue = chnns * safePow(2, m_order) * m_wid;
              // addTimeValue = chnns*(addr-1)*pow(2,m_order)*m_wid;
            }
            if (type_id == 3) // 正负正 + M序列
            {
              addTimeValue =
                  pw_time * 7 * chnns + chnns * safePow(2, m_order) * m_wid;
            }
            // addTimeValue = addTimeValue > 0
            //                    ? (int)((addTimeValue + 500) / 1000 +
            //                    30) : 0;
            addTimeValue =
                addTimeValue > 0 ? (int)((addTimeValue + 500) / 1000) : 0;
          }

          addTimeValue *= 1.5; // 需要考虑数据上传的时间

          uint32_t *upTable = (uint32_t *)(data + 182);

          for (int i = 0; i < timesMap[times_id]; i++) {
            upTable[i] = htobe32(be32toh(upTable[i]) + addTimeValue);
          }
        }

        return setSendBuf(pBuf.get(), addr, cmdId, cmdSeq, data);
      })
      .and_then([&](auto r) {
        std::vector<string> msg({"下发命令字:", std::to_string((unsigned)cmdId),
                                 ",设备地址:", std::to_string(addr),
                                 ",字节数:", std::to_string(bytes),
                                 ",命令标识:", std::to_string(cmdSeq)});

        if (cmdId != 7 && cmdId != 8 && cmdId != 9) {
          cout << std::dec;
          logInfo(msg);//连接检测帧，对时帧，不记录日志
        }
        if (cmdId == 5) {//停止监测
          b_StartSync = false;
          b_enable = false;
        ...
        if (cmdId == 4) {//启动监测
          ...
          auto rslt = conn.executeQuery(
              "select dev_id,chn_no, ms_trig_thrld,is_ms_trig,chn_label "
              "from "
              "e_chns_config where "
              "type_id=0 order by dev_id, chn_no");
          while (rslt.next()) {
            hzi::msEvtCnf.msThrldsMap.insert_or_assign(
                rslt.getString("chn_label"),
                std::make_pair(rslt.getInt("ms_trig_thrld"),
                               rslt.getInt("is_ms_trig") == 1));
          }

          auto rslt1 =
              conn.executeQuery("select ms_trig_puls_wid, ms_ring_thrshld from "
                                "e_ms_samp_config where "
                                "is_init=0");
          if (rslt1.next()) {
            hzi::msEvtCnf.pulsWid = rslt1.getInt("ms_trig_puls_wid");
            hzi::msEvtCnf.ringThrld = rslt1.getInt("ms_ring_thrshld") / 100.0;
          }

          auto rslt2 = conn.executeQuery(
              "select point from p_points where pnt_id=(select "
              "ms_ahead_pnts_id from e_ms_samp_config where "
              "is_init=0 LIMIT 1)");
          if (rslt2.next()) {
            hzi::msEvtCnf.msAheadPnt = rslt2.getInt("point");
          }

          auto rslt21 = conn.executeQuery(
              "select intvl from p_samp_intvls where intvl_id=(select "
              "ms_samp_intvl_id from e_ms_samp_config where "
              "is_init=0 LIMIT 1)");
          if (rslt21.next()) {
            hzi::msEvtCnf.sampIntvl = rslt21.getInt("intvl");
          }
          auto rslt22 =
              conn.executeQuery("SELECT trig_chns, trig_time_win from "
                                "e_ms_trig_params where id=0");
          if (rslt22.next()) {
            hzi::msEvtCnf.msTrigChns = rslt22.getInt("trig_chns");
            hzi::msEvtCnf.msTrigWin = rslt22.getInt("trig_time_win");
          } else {
            hzi::msEvtCnf.msTrigChns = 3;
            hzi::msEvtCnf.msTrigWin = 300;
          }
          hzi::msEvtCnf.validRange =
              hzi::msEvtCnf.msTrigWin / hzi::msEvtCnf.sampIntvl;

          auto rslt3 = conn.executeQuery(
              "select "
              "id,red,green,blue,ms_vol_thrld,e_vol_thrld,cur_thrld from "
              "p_density_colors order by id desc");
          hzi::densityColors.clear();
          while (rslt3.next()) {
            hzi::densityColors.push_back(DensityColor{
                (uint8_t)rslt3.getInt("id"), (uint8_t)rslt3.getInt("red"),
                (uint8_t)rslt3.getInt("green"), (uint8_t)rslt3.getInt("blue"),
                (float)rslt3.getDouble("ms_vol_thrld"),
                (float)rslt3.getDouble("e_vol_thrld"),
                (float)rslt3.getDouble("cur_thrld")});
          }

          auto rslt4 = conn.executeQuery(
              "select "
              "id,gray,ms_vol_thrld,e_vol_thrld,cur_thrld from "
              "p_density_grays order by id desc");
          hzi::densityGrays.clear();
          while (rslt4.next()) {
            hzi::densityGrays.push_back(DensityGray{
                (uint8_t)rslt4.getInt("id"), (uint8_t)rslt4.getInt("gray"),
                (float)rslt4.getDouble("ms_vol_thrld"),
                (float)rslt4.getDouble("e_vol_thrld"),
                (float)rslt4.getDouble("cur_thrld")});
          }

          auto rslt5 = conn.executeQuery(
              "select x_min,x_max,y_min,y_max,z_min,z_max from e_space");
          if (rslt5.next()) {
            hzi::viewSpace.xMax = rslt5.getDouble("x_max");
            hzi::viewSpace.xMin = rslt5.getDouble("x_min");
            hzi::viewSpace.yMax = rslt5.getDouble("y_max");
            hzi::viewSpace.yMin = rslt5.getDouble("y_min");
            hzi::viewSpace.zMax = rslt5.getDouble("z_max");
            hzi::viewSpace.zMin = rslt5.getDouble("z_min");
          }
          hzi::viewSpace.vtispeed_vector.clear();

          auto rslt6 = conn.executeQuery("select * from e_speed");
          if (rslt6.next()) {
            hzi::viewSpace.evenSpeed = rslt6.getDouble("even_speed");
            hzi::viewSpace.psRatio = rslt6.getDouble("ps_ratio");
            hzi::viewSpace.speed_even = rslt6.getInt("speed_even");
            hzi::viewSpace.vti_layers = rslt6.getInt("vti_layers");
            hzi::viewSpace.enablescan = rslt6.getInt("enable_scan");
            hzi::viewSpace.offset = rslt6.getDouble("offset_speed");
            hzi::viewSpace.vp = rslt6.getDouble("vp");
            hzi::viewSpace.vs = rslt6.getDouble("vs");
            hzi::viewSpace.vc = rslt6.getDouble("vc");

            if (hzi::viewSpace.speed_even == 0 &&
                hzi::viewSpace.vti_layers > 0) {
              auto conn2 = hzi::config.pConnPool->getConnection();

              auto rslt7 = conn2.executeQuery("select * from e_vti_speeds");
              while (rslt7.next()) {
                VtiSpeed vtispeed;
                vtispeed.layer_no = rslt7.getInt("layer_no");
                vtispeed.start_loc = rslt7.getDouble("start_loc");
                vtispeed.end_loc = rslt7.getDouble("end_loc");
                vtispeed.p_speed = rslt7.getDouble("p_speed");
                vtispeed.s_speed = rslt7.getDouble("s_speed");
                hzi::viewSpace.vtispeed_vector.push_back(vtispeed);
              }
            }
          }

          auto rslt8 = conn.executeQuery(
              "select chn_label, state_id from e_chns_config");
          while (rslt8.next()) {
            hzi::chnState[rslt8.getString("chn_label")] =
                rslt8.getInt("state_id");
          }
        }
        // 输出置参命令帧，用于查错
        // if (cmdId == 3) {
        //     string file("/tmp/cmd3_buf");
        //     file += std::to_string(addr) + ".dat";
        //     data::writeDataToFile(file.c_str(),
        //                           pBuf.get(),  // +16只看数据区
        //                           hzi::config.sendFrmBytes[cmdId]);
        //     // cout << "dev" << (unsigned)addr << " emit time:"
        //     //      << be32toh(*(uint32_t*)((char*)pBuf.get() + 198)) <<
        //     //      "\n";
        // }

        return cmdId == 8 || cmdId == 9
                   ? sock::write(hzi::config.devSockMap[addr], pBuf.get(),
                                 hzi::config.sendFrmBytes[cmdId], 6)//对时数据包传输高优先级
                   : (hzi::config.linkDevice
                          ? sock::write(hzi::config.devSockMap[addr],
                                        pBuf.get(),
                                        hzi::config.sendFrmBytes[cmdId])
                          : relayConfig(cmdId, addr, pBuf.get(),
                                        hzi::config.sendFrmBytes[cmdId]));
      })
      .and_then([&](auto r) { return tl::expected<uint32_t, string>(cmdSeq); })

      .or_else([&](auto err) {
        debugLog(err);
        return tl::expected<uint32_t, string>(tl::make_unexpected(
            "sendCmd error for cmdId:" + to_string(cmdId) +
            " to device address:" + to_string(addr) + ": " + err));
      });
}
...
├────────────────────────────────────────────────────────────────────────────┤
│ 1. update_all.sh  (调度者)                                                 │
│     ├── 遍历 customers.json → 每台目标机                                    │
│     │    ├─ ssh 探测端口                                                   │
│     │    ├─ 调用 deploy.sh   → 真正干活                                     │
│     │    └─ 记录日志 update_log.txt                                         │
│                                                                             │
│ 2. deploy.sh  (远程安装者)                                                 │
│     ├── rsync 推送 libs 与 release/VER.tar.gz                               │
│     ├── 按需生成 systemd 服务文件 *.service                                 │
│     └── 远程执行 setup.sh $DIR $VER $DB [onlyweb]                           │
│                                                                             │
│ 3. setup.sh  (现场安装者)                                                  │
│     ├── 解压 /opt/DIR/VER.tar.gz → /opt/DIR/VER                             │
│     ├── 迁移旧配置、底图、数据库                                           │
│     ├── 初始化或升级数据库                                                │
│     ├── 生成/刷新 systemd 服务并启动                                       │
│     └── 后台启动 start_semt.sh 采集或 Web                                   │
└────────────────────────────────────────────────────────────────────────────┘
...
│ 4. start_semt.sh  (启动器)                                               │
│     ├── 杀掉旧进程（同目录同名进程）                                       │
│     ├── 清理缓存 etc/last_samp_times.json                                  │
│     ├── 根据 enable_http 决定是否后台启动 hzisemt / hzisemt -c …          │
│     └── 调用 start_monitor（来自 common.sh）                               │
│                                                                             │
│ 5. common.sh  (通用函数库)                                               │
│     ├── log_msg  日志 + 微信                                               │
│     └── start_monitor  通过 HTTPS 接口：                                   │
│         ├─ 基站对时（cmds/2）                                              │
│         ├─ 下发配置（set_config_from_db）                                  │
│         └─ 开始采集（cmds/4）                                              │
│                                                                             │
│ 6. check_restart.sh  (守护 / 自愈脚本，每 1-2 min cron)                    │
│     ├── 探活 HTTP /ping.html → 无响应则 restart_http_server                │
│     ├── 进程守护：缺 hzisemt 采集 / Web → restart_sampling / restart_http  │
│     ├── monitoring()  检查基站在线 & 采样时间                              │
│     ├── check_ms_event()  去抖：10 秒 5 次同一通道视为干扰 → 取消触发      │
│     ├── enable_trigs()   信号过小 → 重新打开通道触发                       │
│     └── 对 DEVS_TO_REMONITOR 列表逐一 restart_monitor + 微信结果           │
└────────────────────────────────────────────────────────────────────────────┘
...
├────────────────────────────────────────────────────────────────────────────┤
│ 7. db_backup.sh  (备份守护)                                               │
│     ├── 00:00 全量 mariabackup --target-dir=YYYY/MM/DD/full                 │
│     ├── */10 分增量 mariabackup --incremental-basedir=最新成功目录          │
│     ├── 保留 7 天自动清理                                                  │
│     └── 写 /var/db_bkups/backup.log                                        │
└────────────────────────────────────────────────────────────────────────────┘
```

一句话总结  
- **update_all + deploy + setup**：完成 **“一次性的版本发布/升级”**。  
- **start_semt + common**：把程序 **“拉起来”** 并 **“告诉基站干活”**。  
- **check_restart**：每 1-2 分钟 **“守护进程 & 基站健康”**，异常自愈并微信告警。  
- **db_backup**：完全独立的 **“数据库备份”** 守护，与业务无耦合。



```shell
# update_all.sh
# 把 release/版本号 目录里的内容，通过 ssh 分发到 customers.json 中定义的各台服务器上，并调用 deploy.sh 完成真正的“升级”动作
...
function print(){
    echo $(cur_time)$1 >> $logfile
}

if [ ! -d release/$VER ]; then
    print "版本'${VER}'不存在,请检查!"
    exit 1
fi
...
default_sub_dir=$(libs/jq -r '.sub_dir' $settings)
default_db_name=$(libs/jq -r '.db_name' $settings)
default_host=$(libs/jq -r '.host' $settings)



echo -e "\n$(cur_time)升级'${VER}'..." >> $logfile
readarray -t users < <(libs/jq -c '.users[]' $settings)
for user in "${users[@]}"; do
    name=$(echo $user | libs/jq -r '.name')
    ssh_port=$(echo $user | libs/jq -r '.ssh_port')
    sub_dir=$(echo $user | libs/jq -r '.sub_dir // "'$default_sub_dir'"')
    db_name=$(echo $user | libs/jq -r '.db_name // "'$default_db_name'"')
    host=$(echo $user | libs/jq -r '.host // "'$default_host'"')

    if ! nc -z -w $timeout $host $ssh_port; then
        print "$name不可访问，请检查"
        continue
    fi

    if ssh -n $host -p $ssh_port "[ -d '/opt/hzisemt/$VER' ]"; then
        print "$name上版本$VER已经存在"
        continue
    fi

    echo -n "$(cur_time)升级$name......" >> $logfile
    ./deploy.sh $VER $host $ssh_port $sub_dir $db_name
    deploy_status=$?
...
```shell
# deploy.sh
# 把 release/VER.tar.gz 同步到远端，按需生成 systemd 服务，然后在远端执行 setup.sh 完成升级。

#!/bin/bash
if [ $# -ne 5 ] && [ $# -ne 6 ]; then 
    echo "用法：$0 版本号 目的主机 ssh端口号 子目录名 数据库名 onlyweb(可选)"
    exit 1
fi
...
DIR=$4
DB=$5
echo "检查安装目录和数据目录，首次安装会创建目录"
ssh -T -p$PORT  $OTHER  "sudo bash -c '[ ! -d /opt/$DIR ] && sudo mkdir -p /opt/$DIR && sudo chown hziadmin:hziadmin /opt/$DIR' "
ssh -T -p$PORT  $OTHER  "sudo bash -c '[ ! -d /var/$DIR ] && sudo mkdir -p /var/$DIR && sudo chown hziadmin:hziadmin /var/$DIR' "

echo "更新库文件..."
rsync -e "ssh -p$PORT" -a ./libs release/$VER $OTHER:/opt/$DIR/
echo "传输安装包,请稍候..."
rsync -e "ssh -p$PORT" -a release/$VER.tar.gz $OTHER:/opt/$DIR/


echo "检查远程主机的 $DIR.service 文件是否存在..."
SERVICE_EXISTS=$(ssh -p $PORT $OTHER "[ -f /etc/systemd/system/$DIR.service ] && echo 'exists'")

if [ "$SERVICE_EXISTS" != "exists" ]; then
  # 如果服务文件不存在，则生成并上传服务文件
  echo "$DIR.service 文件不存在，正在生成和上传..."

  # 创建服务文件内容，动态设置安装路径并以 hziadmin 用户运行
  SERVICE_FILE_CONTENT="[Unit]
Description=Hzimonitors Backend Service
After=network.target

[Service]
ExecStart=/opt/$DIR/current/py_backend/hzimonitors
WorkingDirectory=/opt/$DIR/current/py_backend
StandardOutput=append:/opt/$DIR/python.log
StandardError=append:/opt/$DIR/python_error.log
User=hziadmin
Restart=always
KillMode=process
Environment=DATA_COLLECTION_MODE=true
Environment=${DIR}_home=/opt/$DIR/current/py_backend
LimitNOFILE=65535
LimitNPROC=65535
...
WantedBy=multi-user.target"
  ...
  # 在本地创建临时服务文件
  echo "$SERVICE_FILE_CONTENT" > /tmp/$DIR.service

  # 上传生成的服务文件
  rsync -a -e "ssh -p $PORT" /tmp/$DIR.service $OTHER:/tmp/ || { echo "服务文件上传失败！"; exit 1; }

  # 在远程主机上移动服务文件到正确的位置
  ssh -p $PORT $OTHER << EOF2
    sudo mv /tmp/$DIR.service /etc/systemd/system/$DIR.service
...
EOF2
  ...
  echo "$DIR.service 文件已存在，跳过生成和上传步骤。"
fi

echo "目的主机上升级系统..."

if [ $# -eq 6 ] && [ "$6" == "onlyweb" ]; then
  ssh -T -p$PORT $OTHER "cd /opt/$DIR/$VER && sudo ldconfig && ./setup.sh $DIR $VER $DB onlyweb"
else
  ssh -T -p$PORT $OTHER "cd /opt/$DIR/$VER && sudo ldconfig && ./setup.sh $DIR $VER $DB"
fi
echo "$OTHER上$VER版本升级完成！"
...
```shell
# setup.sh
# 在远端主机上完成：解压安装包、迁移旧配置/底图、初始化/升级数据库、生成并启动 systemd 服务。

#!/bin/bash
if [ $# -ne 3 ] && [ $# -ne 4 ]; then
    echo "用法：$0 子目录名 版本号 数据库名 onlyweb(可选)"
	exit 1
fi

[[ ! -d /opt/$1 ]] && sudo mkdir /opt/$1 && sudo chown hziadmin:hziadmin /opt/$1
...
CURVER=`realpath /opt/$1/current`

# if update
if [[ -L /opt/$1/current ]] && [[ ! $CURVER == /opt/$1/$2 ]]; then 
	rsync -a /opt/$1/current/etc/sys_config.json /opt/$1/$2/etc/ 
	if [[ -f /opt/$1/current/etc/conf_only_web.json ]]; then
		rsync -a /opt/$1/current/etc/conf_only_web.json /opt/$1/$2/etc/
	fi
	if [[ -d /opt/$1/current/py_backend ]]; then
		rsync -a /opt/$1/current/py_backend/etc /opt/$1/$2/py_backend/ 
	fi
fi

CURDIR=`pwd`

ln -sfn /opt/$1/$2 current
cd /opt/$1/current

CONFIG_FILE="etc/sys_config.json"
ONLY_WEB_FILE="etc/conf_only_web.json"

if [[ ! -f $ONLY_WEB_FILE ]]; then
	cp "$CONFIG_FILE" "$ONLY_WEB_FILE"
	../libs/jq '.link_device = false | .time_sync = false | .enable_http = true' \
				"$ONLY_WEB_FILE" >"${ONLY_WEB_FILE}.tmp" && mv "${ONLY_WEB_FILE}.tmp" "$ONLY_WEB_FILE"
	../libs/jq '.enable_http = false' "$CONFIG_FILE" >"${CONFIG_FILE}.tmp" && mv "${CONFIG_FILE}.tmp" "$CONFIG_FILE"
fi

mysql_url=$(../libs/jq -r '.mysql_url' etc/sys_config.json)
if [ $# -eq 4 ] && [ $4 == "onlyweb" ]; then
	mysql_url=$(../libs/jq -r '.mysql_url' etc/conf_only_web.json)
fi
...
user=$(echo $query | cut -d'&' -f1 | cut -d'=' -f2)
password=$(echo $query | cut -d'&' -f2 | cut -d'=' -f2)

SOCKET=$(echo $mysql_url | grep -o 'socket=[^&]*')
[[ -n $SOCKET ]] && SOCKET="--$SOCKET"
...
if [ -d "$CURVER" ]; then
    ...
    while IFS=$'\t' read -r img_name; do
        myArray+=("$img_name")
    done < <(sudo mysql -u$user -P$port -p$password "$SOCKET" -e "use $3; SELECT img_name FROM e_bg_images" --batch --skip-column-names)

    # 处理 e_home_carousel 表
    while IFS=$'\t' read -r img_name; do
        myArray+=("$img_name")
    done < <(sudo mysql -u$user -P$port -p$password "$SOCKET" -e "use $3; SELECT img_name FROM e_home_carousel" --batch --skip-column-names)

    old_path="${CURVER}/assets/images" 
    new_path="/opt/$1/$2/assets/images" 

    # 确保目标目录存在
    mkdir -p "$new_path"

    for img in "${myArray[@]}"; do
        if [ -f "$old_path/$img" ] && [ ! -f "$new_path/$img" ]; then
            cp "$old_path/$img" "$new_path/"
            chmod 755 "$new_path/$img"
        fi
    done
fi

cd /opt/$1/current
TABLES=$(sudo mysql -u$user -P$port -p$password $SOCKET -e "use $3; show tables")
if [[ $TABLES == '' ]]; then
	sudo mysql -u$user -P$port -p$password $SOCKET -e "use $3; source /opt/$1/current/all_tables.sql;"
fi

sudo mysql -u$user -P$port -p$password $SOCKET -e "use $3; source /opt/$1/current/update_db.sql;"

if [ ! -f /etc/systemd/system/$1.service ]; then
	sudo sed "s/hzisemt/$1/g" etc/hzisemt.service | sudo tee /etc/systemd/system/$1.service > /dev/null
...
if [ ! -f /etc/systemd/system/${web_service}.service ]; then
	sudo sed "s/hzisemt/$1/" etc/hzisemt_web.service | sudo tee /etc/systemd/system/${web_service}.service > /dev/null
	sudo systemctl unmask $web_service.service
	sudo systemctl daemon-reload
...
if [ $# -lt 4 ]; then
	./start_semt.sh 1>>../semt.log 2>>../error.log &
fi
...
```shell
# start_semt.sh
# 先杀掉“同一个脚本”或“同一项目目录下”正在跑的进程（防止重复启动）。
# 清掉缓存文件 etc/last_samp_times.json。
# 根据参数决定是否以后台方式启动主进程 hzisemt（或 hzisemt -c etc/conf_only_web.json）。
# 记录日志，最后调用 start_monitor（common.sh 里的函数）做真正的监控初始化。
...
log_msg "结束当前监测进程,准备升级"
for pid in $(pgrep -uhziadmin hzisemt); do
	DIR=$(sudo pwdx $pid | sed s/^.*:\\s//)
	if [[ "$DIR" != "" && $(dirname $DIR) == $(dirname $PWD) ]]; then
			kill $pid
			# web_running=$(cat /proc/$pid/cmdline | tr '\0' ' ' | grep only_web)
			# if [[ $web_running == '' ]]; then
			# 	kill $pid
			# 	log_msg "结束当前监测进程,准备升级"
			# fi
...
rm -f etc/last_samp_times.json

[[ $# -eq 0 ]] && ./hzisemt & 

# [[ -f etc/conf_only_web.json ]] && ./hzisemt -c etc/conf_only_web.json &
# sudo systemctl restart $web_service.service

realpath=$(realpath .)
version=$(echo "$realpath" | awk -F'/' '{print $NF}')


log_msg "版本${version}启动监测"
sleep 20
start_monitor
...
```shell
# common.sh
# 1. 提供 log_msg 日志封装（可联动企业微信）。
# 2. 提供 start_monitor 函数，通过 HTTPS 接口把“基站对时 / 下发配置 / 开始采集”这一套流程跑一遍。
...
CURRTIME=""
HTTP_PORT=`cat etc/sys_config.json | ../libs/jq '.http_port'`
...
function start_monitor(){
	if [[ $# > 0 ]]; then
		echo "$(curr_time) 准备重启${1}号基站监测..."
		curl -X POST https://ahhzi.com.cn:$HTTP_PORT/cmds/2/$1 --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem >/dev/null 2>&1
		echo "$(curr_time) 基站对时完毕"

		curl -X POST https://ahhzi.com.cn:$HTTP_PORT/set_config_from_db/$1 --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem >/dev/null 2>&1
		echo "$(curr_time) ${1}号站下发采集参数完成"
		curl -X POST https://ahhzi.com.cn:$HTTP_PORT/cmds/4/$1 --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem >/dev/null 2>&1
		echo "$(curr_time) 监测数据实时采集开始"
	else
		echo "$(curr_time) 准备重启所有基站监测..."
		curl -X POST https://ahhzi.com.cn:$HTTP_PORT/cmds/5 --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem >/dev/null 2>&1
		echo "$(curr_time) 停止当前数据采集..."
		sleep 5
		curl -X POST https://ahhzi.com.cn:$HTTP_PORT/cmds/2 --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem >/dev/null 2>&1
		echo "$(curr_time) 基站对时完毕"
		curl -X POST https://ahhzi.com.cn:$HTTP_PORT/set_config_from_db --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem >/dev/null 2>&1
		echo "$(curr_time) 下发采集参数完成"
		curl -X POST https://ahhzi.com.cn:$HTTP_PORT/cmds/4 --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem >/dev/null 2>&1
		echo "$(curr_time) 监测数据实时采集开始"
	fi
}
...
```shell
# check_restart.sh
# 最终落到 crontab 里每 1~2 分钟跑一次的 “守护/自愈脚本”
# 探活：HTTP 服务是否返回 pong
# 采集状态巡检：哪些基站长时间不刷新数据
# 微震事件去抖：同一通道 10 秒内连发 5 次视为干扰
# 触发通道自动恢复：信号太小就重新打开 is_ms_trig
# 进程守护：缺哪个进程就补哪个（采集 / Web）
# 异常自愈：重启失败立即通过微信告警
...
#ENABLE_HTTP==false时，说明采集和web是分开运行的
ENABLE_HTTP=`cat etc/sys_config.json | ../libs/jq '.enable_http'`

declare -a DEVS_TO_REMONITOR
declare -a DEVS_ONLINE
#基站状态记录
currStateFile="etc/current_state.txt"
touch "$currStateFile"

# kill http server
function restart_http_server(){
	log_msg "web后台服务无反馈，重启中..."
	if [[ "$ENABLE_HTTP" == "false" ]]; then
	  service_name=$(basename "$(dirname "$(pwd)")_web")
		systemctl status "$service_name" > /dev/null 2>&1
		if [ $? -eq 4 ]; then
				for pid in $(pgrep -uhziadmin hzisemt); do
					if [[ $(sudo pwdx $pid | sed s/^.*:\\s//) == $PWD ]]; then
						web_running=$(cat /proc/$pid/cmdline | tr '\0' ' ' | grep only_web)
						[[ $web_running != '' ]] && kill -9 $pid
					fi
				done
				./hzisemt -c etc/conf_only_web.json &
		else
...
# 基站状态检测、联通的基站是否都在采集监测中
function monitoring(){
	STATIONS=`curl -s --max-time 5 https://ahhzi.com.cn:$HTTP_PORT/stations --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem`
	DEV_STATES=`curl -s --max-time 5 https://ahhzi.com.cn:$HTTP_PORT/ping_device --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem`

	#获取后台当前各基站采集时间
	SAMP_TIMES=`curl -s --max-time 5 https://ahhzi.com.cn:$HTTP_PORT/last_samp_times --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem`
	echo $SAMP_TIMES > etc/last_samp_times.json

	#临时文件
	tempStateFile=$(mktemp)

	DEV=1
	while [[ "$DEV" -le "$STATIONS" ]]; do
		STATUS=`echo $DEV_STATES | ../libs/jq ".\"$DEV\""`
		LAST=`echo $LAST_TIMES | ../libs/jq ".\"$DEV\""`
		CURR=`echo $SAMP_TIMES | ../libs/jq ".\"$DEV\""`

		prevStatus=$(grep "^${DEV}:" "$currStateFile" | cut -d':' -f2)
		if [[ "$STATUS" == "false" ]]; then
			echo "${DEV}:离线" >> "$tempStateFile"
			currStatus="离线"
		else
			echo "${DEV}:上线" >> "$tempStateFile"
			currStatus="上线"
			DEVS_ONLINE+=("$DEV")
		fi

		if [[ "$currStatus" != "$prevStatus" ]]; then
			log_msg "${DEV}号基站${currStatus}" true
		fi

		CURR_MS=`date +%s%3N`
		elapsed=$((CURR_MS - LAST))
		#超过5分钟不再重复发送微信消息
		[[ $elapsed -gt 300000 ]] && toSend="false" || toSend="true"
		if [[ "$STATUS" == "true" && "$LAST" == "$CURR" ]]; then
			msg="${DEV}号站监测停止,试图重启该基站监测"

			if ! [[ " ${DEVS_TO_REMONITOR[*]} " =~ " $DEV " ]]; then
				DEVS_TO_REMONITOR+=("$DEV")
			fi
	...
	mv "$tempStateFile" "$currStateFile"
}


# 重启通道触发
function enable_trigs(){
	url="https://ahhzi.com.cn:$HTTP_PORT/table/e_chns_config/chn_label:str,ms_trig_thrld/type_id=0 and state_id=0 and is_ms_trig=0"
	UNTRIGGED=`curl -s "${url// /%20}" --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem`

	# Parse labels and thresholds into an associative array
	declare -A labels_thresholds
	while IFS= read -r line; do
		chn_label=$(echo "$line" | ../libs/jq -r '.chn_label')
		ms_trig_thrld=$(echo "$line" | ../libs/jq -r '.ms_trig_thrld')
		labels_thresholds["$chn_label"]=$ms_trig_thrld
	done < <(echo $UNTRIGGED | ../libs/jq -c '.data[]')

	if [ ${#labels_thresholds[@]} -eq 0 ]; then
			return
	fi

	# Array to hold labels where max_val < ms_trig_thrld
	declare -a filtered_labels

	# Fetch max_min for each label and compare with ms_trig_thrld
	for label in "${!labels_thresholds[@]}"; do
		# Fetch the max_min using the API
		url="https://ahhzi.com.cn:$HTTP_PORT/table/e_data_digest/abs(max_min),chn_label:str/class_id=3 and chn_label='$label' order by samp_time desc limit 1" 

		result=$(curl -s "${url// /%20}" --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem)
		max_val=$(echo $result | ../libs/jq -r '.data[0]."abs(max_min)"')

		 # Ensure max_val is a valid number
		if ! [[ "$max_val" =~ ^[0-9]+$ ]]; then
			echo "Invalid max_val: $max_val"
			continue
		fi

    	max_vol=$(( max_val * 4000 / 8388607 )) #适当提高检测值

		# Compare max_val with ms_trig_thrld
		if [[ "$max_vol" -lt "${labels_thresholds[$label]}" ]]; then
	...
	# Output the filtered labels
	for label in "${filtered_labels[@]}"; do
		URL="https://ahhzi.com.cn:$HTTP_PORT/update_columns/e_chns_config/chn_label='$label'"
		curl -X POST -s "${URL// /%20}" -H 'Content-Type: application/json' -d '{"updates": "is_ms_trig=1"}' --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem >/dev/null
		dev=${label:1:1}
		if [[ " ${DEVS_ONLINE[*]} " =~ " $dev " ]] && ! [[ " ${DEVS_TO_REMONITOR[*]} " =~ " $dev " ]]; then
        	DEVS_TO_REMONITOR+=("$dev")
    	fi
		log_msg "基站${dev}通道${label}恢复触发"
	done

}
# 微震事件有效性检查
# 规则：最近5个事件为同一基站同一检波器在10秒内触发，认为是干扰产生的无效事件
function check_ms_event(){
	URL="https://ahhzi.com.cn:$HTTP_PORT/table/e_data_frm_info/floor(unix_timestamp(samp_time)),flag,emit_frm_no/class_id=2 order by samp_time desc limit 5"
	RSLT=`curl -s "${URL// /%20}" --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem`
	TIMES=`echo $RSLT | ../libs/jq '.data | map(."floor(unix_timestamp(samp_time))")' | sed -e 's/\[//' -e 's/\]//' -e 's/,//' | sort | sed -z 's/\n/ /g;s/\s*$//;s/^\s*//'`
	TIMES=( $TIMES )
	TLEN=${#TIMES[@]}
	...
	#最后一个事件为5分钟之前，返回
	if [[ $(( $CURR - $LAST )) -gt 300 ]]; then
		return
	fi
	DIFF=$(( $LAST - ${TIMES[0]} ))

	DEVIDS=`echo "$RSLT" | ../libs/jq '.data | map(.flag)' | sed -e 's/\[//' -e 's/\]//' -e 's/,//' | sort -u | uniq | sed -z 's/\n/ /g;s/\s*$//;s/^\s*//'`
	DEVIDS=( $DEVIDS )
	DLEN=${#DEVIDS[@]}
	...
	CHNS=`echo $RSLT | ../libs/jq '.data | map(.emit_frm_no)' | sed -e 's/\[//' -e 's/\]//' -e 's/,//' | sort -u | uniq | sed -z 's/\n/ /g;s/\s*$//;s/^\s*//'`
	CHNS=( $CHNS )
	CLEN=${#CHNS[@]}
	...
	#通道号为0，阈值触发时，下位机未设置触发通道号，返回
	if [[ ${CHNS[0]} -lt 1 ]]; then
		return
	fi

	if [[ $DLEN -eq 1 && $CLEN -eq 1 && $DIFF -lt 10 ]]; then
		DEV=${DEVIDS[0]}
		CHN=${CHNS[0]}
		msg="基站$DEV通道$CHN连续密集触发微震事件，取消触发"
		log_msg $msg
		URL="https://ahhzi.com.cn:$HTTP_PORT/update_columns/e_chns_config/dev_id=$DEV and chn_no=$CHN"
		curl -X POST -s "${URL// /%20}" -H 'Content-Type: application/json' -d '{"updates": "is_ms_trig=0"}' --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem
		if [[ " ${DEVS_ONLINE[*]} " =~ " $dev " ]] && ! [[ " ${DEVS_TO_REMONITOR[*]} " =~ " $dev " ]]; then
        	DEVS_TO_REMONITOR+=("$dev")
    	fi
		# start_monitor $DEV
...
PONG=`curl -s --max-time 5 https://ahhzi.com.cn:$HTTP_PORT/html/ping.html --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem`
...
if [[ $ENABLE_HTTP == false ]]; then
	...
	for pid in $(pgrep -uhziadmin hzisemt); do
		if [[ $(sudo pwdx $pid | sed s/^.*:\\s//) == $PWD ]]; then
			web_running=$(cat /proc/$pid/cmdline | tr '\0' ' ' | grep only_web)
			[[ $web_running != '' ]] && HTTPING=1
			[[ $web_running == '' ]] && SAMPLING=1
		fi
	done

	[[ $HTTPING == 0 ]] && ./hzisemt -c etc/conf_only_web.json &
	[[ $SAMPLING == 0 ]] && restart_sampling
else
	for pid in $(pgrep -uhziadmin hzisemt); do
		if [[ $(sudo pwdx $pid | sed s/^.*:\\s//) == $PWD ]]; then
			SAMPLING=1
...
#读取历史采集时间
if [[ -f etc/last_samp_times.json ]]; then
	LAST_TIMES=`cat etc/last_samp_times.json`
else
	LAST_TIMES=''
fi
...
for DEV in "${DEVS_TO_REMONITOR[@]}"; do
	LAST=`echo $LAST_TIMES | ../libs/jq ".\"$DEV\""`
	CURR_MS=`date +%s%3N`
	elapsed=$((CURR_MS - LAST))
	[[ $elapsed -gt 300000 ]] && toSend="false" || toSend="true"
	start_monitor $DEV $toSend
	#查看重启监测是否成功
	sleep 60
	SAMP_TIMES=`curl -s https://ahhzi.com.cn:$HTTP_PORT/last_samp_times --cacert etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem`
	CURR=`echo $SAMP_TIMES | ../libs/jq ".\"$DEV\""`
	dif=$((CURR - LAST))

	[[ $dif -gt 0 ]] &&	ret="成功" || ret="失败"
	send_message "msg" " 重启${DEV}号基站监测$ret" $toSend
done
...
# 备份策略：
# 每天1次全量备份，每10分钟1次增量备份，保留1周备份数据

BKROOT=/var/db_bkups

if [[ ! -d $BKROOT ]]; then
	sudo mkdir $BKROOT
	sudo chown hziadmin:staff $BKROOT
fi

LOGFILE=$BKROOT/backup.log
...
BACKUP='mariabackup -H 127.0.0.1 --port=3307 --backup --user=hzidba --password="dzaqZHk3" --socket=/var/lib/mysql2/node2.sock '

DAY_DIR=$BKROOT/`date +%Y`/`date +%m`/`date +%d`
FULL_TARGET=$DAY_DIR/full
INC_ROOT=$DAY_DIR/incs
...
if [[ ! -d $FULL_TARGET ]]; then
	get_time
	TIME1=`date +%s`
	mariabackup -H 127.0.0.1 --port=3307 --backup --user=hzidba --password="dzaqZHk3" --socket=/var/lib/mysql2/node2.sock --target-dir=$FULL_TARGET/
	TIME2=`date +%s`
	# TIME=$(( $TIME2-$Time1 ))
	# echo "$CURRTIME 全量备份用时：$TIME秒" >> $LOGFILE
	...
	if [[ ! $LS == '' ]]; then
		...
		INC_BASE=$INC_ROOT/$MAX/
	fi
	HR_MIN=`date +%H%M`
	mariabackup -H 127.0.0.1 --port=3307 --backup --user=hzidba --password="dzaqZHk3" --socket=/var/lib/mysql2/node2.sock --target-dir=$INC_ROOT/$HR_MIN/ --incremental-basedir=$INC_BASE
fi
...
```cpp
//swt_common.hpp
#ifndef MS_TUNNEL_COMMON_HPP
#define MS_TUNNEL_COMMON_HPP
//随掘
...
//3:采集背景帧; 6:采集全时帧; 9:互干涉源地震信号; 11:自干涉源地震信号; 10:偏移结果数据(间隔:网格大小,点数:x网格数,通道数:z网格数);
namespace ms_swt
{
    #define DELBFRS(var) if(var!=nullptr) {delete[] var;var=nullptr;}
    #define DELBFR(var) if(var!=nullptr) {delete var;var=nullptr;}
    enum SwtDataFrameType
    {
        BakgdExtrm = 3, //背景(极值)
        FullTime = 6,   //全时
        CorrItfe = 9,   //互干涉
    ...
    //获取DataFrame基本信息
    float GetDataDeltT(const DataFrame& df);//采样间隔
    int GetChannels(const DataFrame& df);//总通道数
    int GetPoints(const DataFrame& df);//采样点数
    int GetAdvnPoints(const DataFrame& df);//超前点数
    int GetShotCount(const DataFrame& df);//炮数/组数
    ...
    string SaveDataFrameToQfxFile(const string& filepath, const DataFrame& df);
    ...
    //计算波形数据周期点范围
    void CalWavePeriod(const float* pData, int minPos, int maxPos, int curPt, int& begPt, int& endPt,bool halfPeriod = true);
    //道均衡
    void TraceEqualize(vector<MSDataTrace*>& vtTrc, int points, int equalWay, int begId,int EndId);

};//namespace

#endif//MS_TUNNEL_COMMON_HPP
```
```cpp
//swt_common.cpp
...
namespace ms_swt
{
    float GetDataDeltT(const DataFrame& df)
    {
        float deltT = 1;
        int classId = *((uint8_t *)(df.upHead.get() + 8));
        ...
        {
            deltT = *((float*)(df.upHead.get() + 9));
        }
        else if (SwtDataFrameType::MigrImage==classId)
        {
            deltT = *((float*)(df.upHead.get() + 24));
        }

        return deltT;
    }
    int GetChannels(const DataFrame& df)
    {
        int chns = 0;
        int classId = *((uint8_t *)(df.upHead.get() + 8));
        ...
        {
            chns = *((uint32_t*)(df.upHead.get() + 13));
        }
        else if (SwtDataFrameType::MigrImage==classId)
        {
            chns = *((uint16_t*)(df.upHead.get() + 14));
        }

        return chns;
    }
    int GetPoints(const DataFrame& df)
    {
        int points = 0;
        int classId = *((uint8_t *)(df.upHead.get() + 8));
        ...
        {
            points = *((uint32_t*)(df.upHead.get() + 19));
        }
        else if (SwtDataFrameType::MigrImage==classId)
        {
            points = *((uint16_t*)(df.upHead.get() + 10));
        }

        return points;
    }
    int GetAdvnPoints(const DataFrame& df)
    {
        int points = 0;
        int classId = *((uint8_t *)(df.upHead.get() + 8));
        if (SwtDataFrameType::CorrItfe==classId || SwtDataFrameType::MigrImage==classId)
        {
            points = *((uint32_t*)(df.upHead.get() + 23));
        }
        return points;
    }
    int GetShotCount(const DataFrame& df)
    {
        int shots = 1;
        int classId = *((uint8_t *)(df.upHead.get() + 8));
        if (SwtDataFrameType::CorrItfe==classId || SwtDataFrameType::AcorrItfe==classId)
        {
            shots = *((uint16_t*)(df.upHead.get() + 17));
        }
        return shots;
    }
    bool GetDataBasicInfoFromDataFrameHead(const char* pHead,int& chns, float& deltT,int& points,int& advPoints,int& shots)
    ...
    string SaveDataFrameToQfxFile(const string& filepath, const DataFrame& df)
    {
        ...
        try
        {
            std::ofstream outQfx(filepath);
            STQfxFileHead fhQfx;
            STQfxTraceHead thQfx;
            int classId = df.classId();
            ms_swt::GetDataBasicInfoFromDataFrameHead(df.upHead.get(),fhQfx.nTraceCount,fhQfx.fSampleInterval,fhQfx.nSamplePoints,fhQfx.nAdvanPoints,fhQfx.nShotCount);
            outQfx.write(reinterpret_cast<char *>(&fhQfx), sizeof(STQfxFileHead));
            thQfx.nSamplePoints = fhQfx.nSamplePoints;
            thQfx.fSampleInterva = fhQfx.fSampleInterval;
            int32_t* p32= nullptr;
            pData = new float[fhQfx.nSamplePoints];
            for (int ch = 0; ch < fhQfx.nTraceCount; ch++)
            {
                thQfx.uchTraceType = 1;
                thQfx.nTraceNum = thQfx.nOrgTraceNum = ch + 1;
                outQfx.write(reinterpret_cast<char *>(&thQfx),sizeof(STQfxTraceHead));
                p32 = (int32_t *)df.upData.get() + ch * fhQfx.nSamplePoints;
                if (classId==3 || classId==6) {
                    for (int i = 0; i < fhQfx.nSamplePoints; i++)
                    {
                        pData[i] = p32[i]*hzi::config.voltageCoef;
                    }
                    outQfx.write((char*)pData,sizeof(float)*fhQfx.nSamplePoints);
                }
                else {
                    outQfx.write(df.upData.get()+ch*fhQfx.nSamplePoints*4,sizeof(float)*fhQfx.nSamplePoints);
                }
            }
            outQfx.close();
        }
    ...
    void CalWavePeriod(const float* pData, int minPos, int maxPos, int curPt, int& begPt, int& endPt,bool halfPeriod)
    {
        //判断当前值的符号
        int ampSmbol = pData[curPt]>0?1:(pData[curPt]<0?-1:0);

        int iTempNum = 0;
        float fZero = 0;
        begPt = endPt = curPt;

        //如果当前是正值，则向后找一个过零点,再向前找两个过零点
        if (1 == ampSmbol)
        {
            //向前寻找一个过零点
            for (int i = curPt-1; i >= minPos; --i,++iTempNum)
            {
                if (pData[i]<fZero)
                {
                    break;
            ...
            iTempNum = 0;
            //找到第一个后，再找一个过零点
            if (begPt>minPos && !halfPeriod)
            {
                float fCurAmp = pData[begPt-1];
                for (int i = begPt-1; i>=minPos; --i,++iTempNum)
                {
                    if (pData[i]*fCurAmp < fZero)
                    {
                        break;
                    }
                    begPt = i;
        ...
        //如果当前是零点，则分别向前 向后找一个过零点
        else
        {
            //向前找，即从当前值的前一个值,向前找一反向的
            if (curPt>minPos)
            {
                iTempNum = 0;
                float fCurAmp = pData[curPt-1];
                for (int i = curPt-1; i >= minPos; --i,++iTempNum)
                {
                    if ( pData[i]*fCurAmp < 0)
                    {
                        break;
                    }
                    begPt = i;
            ...
            //向后找，即从当前值的下一个值,向后找一反向的
            if (endPt<maxPos && !halfPeriod)
            {
                iTempNum = 0;
                float fCurAmp = pData[curPt+1];
                for (int i = curPt+1; i < maxPos; ++i,++iTempNum)
                {
                    if ( pData[i]*fCurAmp < 0)
                    {
                        break;
    ...
    void TraceEqualize(vector<MSDataTrace*>& vtTrc, int points, int equalWay, int begId,int EndId)
    {
        float zeroEsp = 1e-5f;
        float* pBfr = nullptr;
        if(equalWay == TraceEqualWay::MaxAmp)
        {
            float fAmpMin, fAmpMax, fAmpTemp;
            //所有道最值
            fAmpMax = vtTrc[0]->Head().ampleMax;
            fAmpMin = vtTrc[0]->Head().ampleMin;
            for (int tr = 1; tr < vtTrc.size(); tr++) {
                fAmpTemp = vtTrc[tr]->Head().ampleMax;
                fAmpMax = fAmpMax < fAmpTemp ? fAmpTemp : fAmpMax;

                fAmpTemp = vtTrc[tr]->Head().ampleMin;
                fAmpMin = fAmpMin > fAmpTemp ? fAmpTemp : fAmpMin;
            }
            fAmpMax = ( fabs(fAmpMax) > fabs(fAmpMin) ? fabs(fAmpMax) : fabs(fAmpMin) );
            fAmpMax = fAmpMax<zeroEsp?1.0f:fAmpMax;

            //w=A(ref)/A(cur)
            pBfr = new float[points];
            for (int tr=0; tr < vtTrc.size(); tr++)
            {
                fAmpTemp = vtTrc[tr]->Head().ampleMax;
                fAmpMin	 = vtTrc[tr]->Head().ampleMin;
                fAmpTemp = ( fabs(fAmpTemp) > fabs(fAmpMin) ? fabs(fAmpTemp) : fabs(fAmpMin) );
                if(fAmpTemp<zeroEsp) {continue;}                
                fAmpTemp = (fAmpMax / fAmpTemp);
                memcpy(pBfr, (vtTrc[tr])->GetData(), sizeof(float)*points);
                for (int i = 0; i < points; i++) {
                    pBfr[i] *= fAmpTemp;
                }
                vtTrc[tr]->SetData(pBfr,points);
            }
        }//maxamp
        else if(equalWay == TraceEqualWay::AvgAmp)
        {
            //所有道均值
            std::vector<float> vtAvg;
            float refAvg = 0;
            double sum = 0;
            float* pTmpData = nullptr;
            for (int tr = 0; tr < vtTrc.size(); tr++) {
                pTmpData = vtTrc[tr]->GetData();
                sum = 0;
                for (int i = 0; i < points; i++) {
                    sum += fabsf(pTmpData[i]);
                }
                sum /= points;
                vtAvg.push_back(sum);
                refAvg += sum;
            ...
            refAvg /= vtTrc.size();
            refAvg = refAvg<zeroEsp?1:refAvg;
...
namespace ms_swt
{
    ...
    string ExtractInterfeSingal(uint64_t fromTimeMs, uint64_t toTimeMs, int lineId,bool isRealtimeExt)
    {
        ...
        DataFrame dfRaw;
        tl::expected<DataFrame, string> dmRlt;
        if (isRealtimeExt)
        {
            dmRlt = ms_swt::DataMerge(fromTimeMs, toTimeMs, lineId, hzi::mining_classId); // 实时使用全局变量
        }
        else
        {
            // 历史查表
            auto conn = hzi::config.pConnPool->getConnection();
            auto rslt0 = conn.executeQuery(
                "SELECT class_Id FROM `e_mining_PCOSignalParm_history`");
            int classId = 6;
            if (rslt0.next())
            {
                classId = rslt0.getInt("class_Id"); // 拼接数据类型（6 微震全时 3-微震背景）
                logInfo("history process data class_id=" + to_string(classId));
            }
            dmRlt = ms_swt::DataMerge(fromTimeMs, toTimeMs, lineId, classId);
        }
    ...
    string ExtractInterfeSingal(MSTraceMgr& xcorrData, uint64_t fromTimeMs, uint64_t toTimeMs, int lineId,bool isRealtimeExt)
    {
        ...
        {
            dfRaw.setHeadValue<double>(0, (double)(dfRaw.sampTime() + 60));
            logInfo("[" + timestampToString(dfRaw.sampTime()) + "]:ExtractInterfeSingal: FullTime data type..");
        }
        else if (dfRaw.classId() == SwtDataFrameType::BakgdExtrm)
        {
            dfRaw.setHeadValue<double>(0, (double)(dfRaw.sampTime() + 30));
            logInfo("[" + timestampToString(dfRaw.sampTime()) + "]:ExtractInterfeSingal: BakgdExtrm data type..");
        }
        // dfRaw.setHeadValue<double>(0, (double)(dfRaw.sampTime() + (isRealtimeExt?1:2)));
        if(isRealtimeExt)
        {
            dfRaw.setHeadValue<double>(0, (double)(dfRaw.sampTime() + 1));
            logInfo("[" + timestampToString(dfRaw.sampTime()) + "]:ExtractInterfeSingal: Realtime Ext..");
        }
        else
        {
            dfRaw.setHeadValue<double>(0, (double)(dfRaw.sampTime() + 2));
            logInfo("[" + timestampToString(dfRaw.sampTime()) + "]:ExtractInterfeSingal: Not Realtime Ext..");
        }
...
```
```cpp
//swt_signal_identify.hpp
#ifndef MS_TUNNEL_SIGNAL_IDENTIFY_HPP
#define MS_TUNNEL_SIGNAL_IDENTIFY_HPP
//随掘地震掘进信号识别
...
namespace ms_swt
{
    ...
    struct tunnel_signal_identify
    {
        /* data */
    };

    void IdentifySignal(uint64_t fromTime, uint64_t toTime, int identifyTimeLen);
};//namespace

#endif//MS_TUNNEL_SIGNAL_IDENTIFY_HPP
```
```cpp
//swt_signal_identify.cpp
#include "swt_signal_identify.hpp"
...
{
    //午夜时间
    int64_t Midnight(uint64_t timestamp, bool isMillSecond) 
    {
        time_t tmt = isMillSecond?(timestamp / 1000):timestamp;
        tm* pTM = localtime(&tmt);
        pTM->tm_hour = 0;pTM->tm_min = 0;pTM->tm_sec = 0;
        return (isMillSecond?(mktime(pTM) * 1000):mktime(pTM));
    }

    //数据拼接,时间为毫秒数
    tl::expected<DataFrame, string> DataMerge(uint64_t fromsampTime,uint64_t endsampTime, int dev_id,int dataClassId)
    {
        if (endsampTime <= fromsampTime)
        {
            return tl::make_unexpected("时间段非法");
        }
        std::vector<string> pathVector;
        auto conn = hzi::config.pConnPool->getConnection();
        double fromtime = double(fromsampTime / 1000);
        ...
        //统计待拼接参数
        auto pnts0 = 0,chns0 = 0;
        int mergeChns = 0, mergePoints = 0;
        mergeChns = chns0 = hzi::config.devicesMap[dev_id].dataChns[2];
        auto rslt = conn.executeQuery(
            "SELECT dev_id, samp_time,channels,ch_pnts, frm_loc FROM e_data_frm_info  "
            " where class_id= ?  AND samp_time > FROM_UNIXTIME(?) "
            "  AND samp_time < FROM_UNIXTIME(?)   ORDER BY "
            "samp_time ASC",
            dataClassId, fromtime, endtime);
        while (rslt.next())
        {
            pnts0 = rslt.getInt("ch_pnts");
            int tmp_devID = rslt.getInt("dev_id");
            ...
            mergePoints += pnts0;
            pathVector.push_back( rslt.getString("frm_loc"));
        }
        if (pnts0 == 0 || mergePoints == 0 || chns0 == 0)
            return (tl::make_unexpected("时间段内无数据"));

        //拼接数据体
        DataFrame mergeDF(mergeChns, mergePoints);
        string path = Config::dataRoot + "/";
        std::unique_ptr<char[]> pHead = std::make_unique<char[]>(32);
        ...
        try
        {
            for (int i = 0; i < pathVector.size(); i++)
            {
                ...
                auto pData = std::make_unique<char[]>(fileSize - 32);
                ifs.read(pHead.get(), 32);
                ifs.read(pData.get(), fileSize - 32);

                //多基站数据帧,dev_id的起始位置
                int devMark = static_cast<unsigned char>(pHead[26]);
                int chnOffset = 0;
                for (int bit = 0; bit < dev_id - 1; ++bit)
                {
                    if (devMark & (1 << bit))
                ...
                if(i==0)
                {
                    memcpy(mergeDF.upHead.get(), pHead.get(), 32);
                    *(uint8_t *)(mergeDF.upHead.get() + 26) = dev_id;
                    mergeDF.setPoints(mergePoints);
                    mergeDF.setChannels(mergeChns);
                }

                //拼接数据帧
                chns0 = *(uint16_t *)(pHead.get() + 10);
                pnts0 = *(uint32_t *)(pHead.get() + 12);
                for (int ch = 0; ch < mergeChns; ++ch)
                {
                    int sourceCh = chnOffset + ch; // 计算文件数据的实际通道位置
                    if (sourceCh < chns0)
                    {
                        memcpy(mergeDF.upData.get() + ch * mergePoints * 4 + i * pnts0 * 4,pData.get() + sourceCh * pnts0 * 4, pnts0 * 4);
                    }
                }
    ...
    //数据拼接,基于通道和采样率的二次采样
    tl::expected<DataFrame, string> DataMerge_SubSample(uint64_t fromsampTime,uint64_t endsampTime, int dev_id,int dataClassId,float mergeDeltT,int mergeChns)
    {
        if (endsampTime <= fromsampTime)
        {
            return tl::make_unexpected("时间段非法");
        ...
        auto pnts0 = 0,chns0 = 0;
        float srcDeltT = 0;
        int mergePoints=0,skipPt = 0,curMergePt=0;
        chns0 = hzi::config.devicesMap[dev_id].dataChns[2];
        auto rslt = conn.executeQuery(
            "SELECT dev_id, samp_time,channels,ch_pnts,samp_intvl,frm_loc FROM e_data_frm_info  "
            " where class_id= ?  AND samp_time > FROM_UNIXTIME(?) "
            "  AND samp_time < FROM_UNIXTIME(?)   ORDER BY "
            "samp_time ASC",
            dataClassId, fromtime, endtime);
        ...
        {
            pnts0 = rslt.getInt("ch_pnts");
            srcDeltT = rslt.getDouble("samp_intvl");
            int tmp_devID = rslt.getInt("dev_id");
            if ((tmp_devID & (1 << (dev_id - 1))) == 0)
            {
                continue; //标记值不包含目标基站
            }
            skipPt =  int((mergeDeltT+srcDeltT/2) / srcDeltT);
            mergePoints += (pnts0/skipPt);
            pathVector.push_back( rslt.getString("frm_loc"));
        }
        if (pathVector.empty())
            return (tl::make_unexpected("时间段内无数据"));
        ...
        string path = Config::dataRoot + "/";
        std::unique_ptr<char[]> pHead = std::make_unique<char[]>(32);
        int32_t* pSrc32=nullptr,*pMerge32=nullptr;
        mergeDF.setHeadValue<float>(19,mergeDeltT);
        *(uint8_t *)(mergeDF.upHead.get() + 26) = dev_id;
        try
        {
            int tmpPtNum = 0;
            for (int ip = 0; ip < pathVector.size(); ip++)
            {
                //读取文件
                auto filePath = path + pathVector[ip];
                if (!fs::exists(filePath))
                {continue;}
                std::ifstream ifs(filePath);
                auto fileSize = fs::file_size(filePath);
                auto pSrc = std::make_unique<char[]>(fileSize - 32);
                ifs.read(pHead.get(), 32);
                ifs.read(pSrc.get(), fileSize - 32);
                ...
                chns0 = *(uint16_t *)(pHead.get() + 10);
                pnts0 = *(uint32_t *)(pHead.get() + 12);
                srcDeltT = *(float *)(pHead.get() + 19);
                skipPt =  int((mergeDeltT+srcDeltT/2) / srcDeltT);
                for (int ch = 0; ch < mergeChns; ++ch)
                {
                    tmpPtNum = 0;
                    pMerge32 = mergeDF.pChnData(ch);
                    pSrc32 = (int32_t*)pSrc.get()+(chnOffset + ch)*pnts0;
                    for (int i = 0; i < pnts0; i+=skipPt,tmpPtNum++)
                    {
                        pMerge32[curMergePt+tmpPtNum] = pSrc32[i];
                    }
                }
                curMergePt += tmpPtNum;
        ...
        {
            std::cout << e.what() << '\n';
            return tl::make_unexpected("数据拼接错误");
        }
    ...
    void IdentifySignal(uint64_t fromTime, uint64_t toTime, int identifyTimeLen)
    {
        //拼接时段数据
        DataFrame rawData;
        auto dmRlt = DataMerge_SubSample(fromTime,toTime,1,6,20,3);
        if (!dmRlt)
        {
           return;
        }
        rawData = dmRlt.value();
        int chns = rawData.channels();
        int points = rawData.points();
        float deltT = rawData.sampIntvl();

        //统计数据信息
        float wndTime = 10000;//10s
        int wndPoint = int(wndTime/deltT);
        int infoPoint = points / wndPoint;
        std::vector<float*> vtInfo;
        for (int i = 0; i < chns; i++)
        {
            vtInfo.push_back(new float[infoPoint]);
        }
        ...
        for (int ch = 0; ch < chns; ch++)
        {
            p32 = rawData.pChnData(ch);
            for (int i = 0; i < points; i++)
            {
                pTmpData[i] = (p32[i]*hzi::config.voltageCoef);
            }
            PureCommonOperation::DataCorrect(pTmpData.get(),points,PureCommonOperation::DANT_DEDC);
            for (int pt = 0; pt < infoPoint; pt++)
            {
                ptIdBeg = pt*wndPoint;
                sumValue = 0;
                for (int i = 0; i < wndPoint; i++)
                {
                    sumValue += (pTmpData[ptIdBeg+i]*pTmpData[ptIdBeg+i]);
                }
                vtInfo[ch][pt] = sqrt(sumValue/wndPoint);
            }
        }

        //识别可能的掘进段
        float tunnelTimeLen = 120000;//s
        float judgeThold = 2;
        float overTholdRatio = 0.4f;
        int infoNumOfSect = int(tunnelTimeLen/wndTime);
        int tunnelSectCount = infoPoint / infoNumOfSect;
        int infoBegId = 0,overTholdNum=0;
        for (int ts = 0; ts < tunnelSectCount; ts++)
        {
            infoBegId = ts * infoNumOfSect;
            overTholdNum=0;
            for (int i = 0; i < infoNumOfSect; i++)
            {
                if (vtInfo[0][infoBegId+i] > judgeThold)
                {
                    overTholdNum++;
                }
            }
            float tunnelFlag=0;
            if (overTholdNum*1.0 / infoNumOfSect > overTholdRatio)
            {
                tunnelFlag=1;
            }
            for (int i = 0; i < infoNumOfSect; i++)
            {
                vtInfo[vtInfo.size()-1][infoBegId+i]=tunnelFlag;
            }
        }
        
        //RMS写文件
        string filepath = "/var/cyli/test/test.csv";
		...
		for(int pt = 0;pt<infoPoint;pt++)
		{
			...
			for(int ch = 0;ch<chns;++ch)
			{
				outFile << vtInfo[ch][pt];
                if(ch!=chns-1)outFile<<",";
			}
			outFile<< std::endl;
		}
		outFile.close();
        ...
        //质量评价
        fromTime = fromTime+10000000;
        toTime = fromTime+30000;
        DataFrame data = DataMerge(fromTime,toTime,1,6).value();
        //DataFrame data = DataMerge_SubSample(fromTime,toTime,1,6,20,16).value();
        chns = data.channels();
        points = data.points();
        deltT = data.sampIntvl();
        filepath = "/var/cyli/test/test.qfx";
        std::ofstream outQfx(filepath);
        STQfxFileHead fhQfx;
        STQfxTraceHead thQfx;
        fhQfx.nTraceCount = chns;
        fhQfx.nSamplePoints = 2048;
        fhQfx.fSampleInterval = deltT;
        fhQfx.nAdvanPoints = 256;
        fhQfx.uchSurveyUnit = QFX_SSU_MV;
        outQfx.write(reinterpret_cast<char *>(&fhQfx), sizeof(STQfxFileHead));
        thQfx.nSamplePoints = fhQfx.nSamplePoints;
        thQfx.fSampleInterva = fhQfx.fSampleInterval;
        auto pCoef = std::make_unique<float[]>(points);
        pTmpData = std::move(std::make_unique<float[]>(points));
        p32 = data.pChnData(0);
        auto pCorrData = std::make_unique<float[]>(fhQfx.nSamplePoints);
        for (int i = 0; i < points; i++)
        {
            pCoef[i] = (p32[i]*hzi::config.voltageCoef);
        }
        PureCommonOperation::DataCorrect(pCoef.get(),points,PureCommonOperation::DANT_DEDC);
        for (int ch = 0; ch < chns; ch++)
        {
            p32 = data.pChnData(ch);
            for (int i = 0; i < points; i++)
            {
                pTmpData[i] = (p32[i]*hzi::config.voltageCoef);
            }
            PureCommonOperation::DataCorrect(pTmpData.get(),points,PureCommonOperation::DANT_DEDC);
            PureCommonOperation::CorrFD(pTmpData.get(),points,pCoef.get(),points,pCorrData.get(),fhQfx.nSamplePoints,fhQfx.nAdvanPoints,PureCommonOperation::BT_UNBIASED);
            thQfx.uchTraceType = 1;
            thQfx.nTraceNum = thQfx.nOrgTraceNum = ch + 1;
            outQfx.write(reinterpret_cast<char *>(&thQfx),sizeof(STQfxTraceHead));
            outQfx.write((char*)pCorrData.get(),sizeof(float)*fhQfx.nSamplePoints);
        }
        outQfx.close();

        //clear
        for (size_t i = 0; i < vtInfo.size(); i++)
        {
            delete[] vtInfo[i];
            vtInfo[i] = nullptr;
        }
...
```
```cpp
//swt_signal_merge.hpp
#ifndef MS_TUNNEL_SIGNAL_MERGE_HPP
#define MS_TUNNEL_SIGNAL_MERGE_HPP
//随掘地震信号拼接
...
namespace ms_swt
{
    tl::expected<DataFrame, string> DataMerge(uint64_t fromsampTimeMs,uint64_t endsampTimeMs, int dev_id = 1, int dataClassId = 6);
    string DataMerge(MSTraceMgr& trcMgr, uint64_t fromsampTimeMs,uint64_t endsampTimeMs, int dev_id = 1, int dataClassId = 6);

    tl::expected<DataFrame, string> DataMerge_SubSample(uint64_t fromsampTimeMs,uint64_t endsampTimeMs, int dev_id,float mergeDeltT,int mergeChns);

};//namespace

#endif//MS_TUNNEL_SIGNAL_MERGE_HPP
```
```cpp
#include "swt_signal_merge.hpp"
#include "Config.hpp"
...
namespace ms_swt
{
    string GetFileOfTimeSpan(uint64_t fromsampTimeMs,uint64_t endsampTimeMs, int dev_id, std::vector<string>& vtFilePath, int& mergeChns, int& mergePoints)
    {
        string ret = "";
        auto conn = hzi::config.pConnPool->getConnection();
        double fromtime = double(fromsampTimeMs / 1000);
        double endtime = double(endsampTimeMs / 1000);
        int dataClassId = SwtDataFrameType::FullTime;
        auto pnts0 = 0,chns0 = 0;
        mergeChns = mergePoints = 0;
        try
        {
            mergeChns = chns0 = hzi::config.devicesMap[dev_id].dataChns[2];
            auto rslt = conn.executeQuery(
                "SELECT dev_id, samp_time,channels,ch_pnts, frm_loc FROM e_data_frm_info  "
                " where class_id= ?  AND samp_time >= FROM_UNIXTIME(?) "
                "  AND samp_time < FROM_UNIXTIME(?)   ORDER BY samp_time ASC",
                dataClassId, fromtime, endtime);
            while (rslt.next())
            {
                pnts0 = rslt.getInt("ch_pnts");
                ...
                mergePoints += pnts0;
                vtFilePath.push_back( rslt.getString("frm_loc"));
            }
            if(vtFilePath.size() == 0)
            {
                ret = "时间段内无数据文件";
            }
        }
    ...
    //数据拼接,时间为毫秒数
    tl::expected<DataFrame, string> DataMerge(uint64_t fromsampTimeMs,uint64_t endsampTimeMs, int dev_id)
    {
        // auto conn = hzi::config.pConnPool->getConnection();
        std::vector<string> vtPath;
        // double fromtime = double(fromsampTimeMs / 1000);
        // double endtime = double(endsampTimeMs / 1000);
        // int dataClassId = SwtDataFrameType::FullTime;
        ...
        //     pathVector.clear();
        // }
        string retMsg = GetFileOfTimeSpan(fromsampTimeMs,endsampTimeMs,dev_id,vtPath,mergeChns,mergePoints);
        if (!retMsg.empty()) {
            return (tl::make_unexpected(retMsg));
        }

        //拼接数据体
        DataFrame dfMerge(mergeChns, mergePoints);
        // dfIntfe.setHeadValue<double>(0,(double)dfRaw.sampTime());
        bool isSetHead = false;
        string path = Config::dataRoot + "/";
        std::unique_ptr<char[]> pHead = std::make_unique<char[]>(32);
        try
        {
            for (int ip = 0; ip < vtPath.size(); ip++)
            {
                auto filePath = path + vtPath[ip];
                if (!fs::exists(filePath))
                {
                    continue;
                }
                ...
                //多基站数据帧,dev_id的起始位置
                int devMark = static_cast<uint8_t>(pHead[26]);
                int chnOffset = 0;
                for (int bit = 0; bit < dev_id - 1; ++bit)
                {
                    if (devMark & (1 << bit))
                ...
                if(!isSetHead)
                {
                    memcpy(dfMerge.upHead.get(), pHead.get(), 32);
                    *(uint8_t *)(dfMerge.upHead.get() + 26) = dev_id;
                    dfMerge.setPoints(mergePoints);

                    // // 若保存的是全时数据的提取数据，则时间戳加60ms
                    // // 若保存的是背景数据的提取数据，则时间戳加30ms
                    // if(dfMerge.classId() == SwtDataFrameType::FullTime)
                    // {
                    //     dfMerge.setHeadValue<double>(0, (double)(dfMerge.sampTime() + 60));
                    // }
                    // else if(dfMerge.classId() == SwtDataFrameType::BakgdExtrm)
                    // {
                    //     dfMerge.setHeadValue<double>(0, (double)(dfMerge.sampTime() + 30));
                    // }
                ...
                for (int ch = 0; ch < mergeChns; ++ch)
                {
                    memcpy(dfMerge.upData.get() + ch * mergePoints * 4 + ip * pnts0 * 4,pData.get() + (chnOffset + ch) * pnts0 * 4, pnts0 * 4);
                }
        ...
        {
            std::cerr << e.what() << '\n';
            cout<<"swt_signal_merge: No data within the specified time!"<<endl;
            return tl::make_unexpected("数据拼接错误");
        }
    ...
    string DataMerge(MSTraceMgr& trcMgr,uint64_t fromsampTimeMs,uint64_t endsampTimeMs, int dev_id)
    {
        std::vector<string> vtPath;
        auto pnts0 = 0,chns0 = 0;
        int mergeChns = 0, mergePoints = 0;
        ...
        trcMgr.Reset();
        MsTraceHead trcHead;
        trcMgr.Head().traceCount = mergeChns;
        trcMgr.Head().samplePoint = mergePoints;
        float* pTempBfr = new float[mergePoints];
        memset(pTempBfr,0,sizeof(float)*mergePoints);
        for (int ch = 0; ch < mergeChns; ch++)
        {
            trcHead.componetNum = trcHead.shotNum = 1;
            trcHead.recvNum = trcHead.traceNum = ch+1;
            trcMgr.AddTrace(trcHead,pTempBfr,mergePoints);
        }
        DELBFRS(pTempBfr)
        ...
        string path = Config::dataRoot + "/";
        std::unique_ptr<char[]> pHead = std::make_unique<char[]>(32);
        std::vector<MSDataTrace*>& vtTrc = trcMgr.GetTraces();
        try
        {
            for (int ip = 0; ip < vtPath.size(); ip++)
            {
                ...
                if(!isSetHead)
                {
                    trcMgr.Head().sampleTime = *(uint64_t*)(pHead.get());
                    trcMgr.Head().deltT = *(float *)(pHead.get() + 19);

                    // if(*(uint8_t *)(pHead.get() + 8) == SwtDataFrameType::FullTime)
                    // {
                    //     trcMgr.Head().sampleTime += 60;
                    // }
                    // else if(*(uint8_t *)(pHead.get() + 8) == SwtDataFrameType::BakgdExtrm)
                    // {
                    //     trcMgr.Head().sampleTime += 30;
                    // }
                ...
                for (int ch = 0; ch < mergeChns; ++ch)
                {
                    pTempBfr = vtTrc[ch]->GetData()+ch*mergePoints+ip*pnts0;
                    int32_t* p32 = (int32_t*)pData.get() + (chnOffset + ch)*pnts0;
                    for (int i = 0; i < pnts0; i++)
                    {
                        pTempBfr[i] = p32[i]*hzi::config.voltageCoef;
                    }
                }
    ...
    //数据拼接,基于通道和采样率的二次采样
    tl::expected<DataFrame, string> DataMerge_SubSample(uint64_t fromsampTime,uint64_t endsampTime, int dev_id,float mergeDeltT,int mergeChns)
    {
        if (endsampTime <= fromsampTime)
        {
            std::cout<<"swt_signal_merge: time is illegal!"<<std::endl;
            return tl::make_unexpected("时间段非法");
        }
        ...
        if (pathVector.empty() || mergePoints < 3*pnts0)
            return (tl::make_unexpected("时间段内无数据"));
        ...
        *(uint8_t *)(mergeDF.upHead.get() + 26) = dev_id;
        ...
        {
            std::cout << e.what() << '\n';
            std::cout<<"swt_signal_merge: No data within the specified time!"<<std::endl;
            return tl::make_unexpected("数据拼接错误");
        }
...
#include "ms_locator.hpp"
using namespace std::chrono;
void sync1(uint8_t devId);
void sync2(uint8_t devId, uint64_t sync1RecvTime);
void syncAllDevices();
...
void syncWarningLeader();
void syncMsevtsKafka();
// namespace time_sync {
// extern u_int64_t t1[2];
...
#include "Config.hpp"
#include "web/Data.hpp"
#include "client_handle_data.hpp"
#include "common.hpp"
#include "handleWarning.hpp"
// namespace time_sync {
// u_int64_t t1[2];
...
void sync1(uint8_t devAddr) {
    auto now = high_resolution_clock::now();
    auto duration = now.time_since_epoch();
    auto microSecs = duration_cast<microseconds>(duration).count();

    auto pBuf = std::make_unique<char[]>(16);
    *(uint64_t*)pBuf.get() = htobe64(microSecs);
    sender::sendCmd(devAddr, 8, (char*)pBuf.get());
}
void sync2(uint8_t devAddr, uint64_t sync1RecvTime) {
    auto pBuf = std::make_unique<char[]>(16);
    *(uint64_t*)pBuf.get() = htobe64(sync1RecvTime);
    sender::sendCmd(devAddr, 9, (char*)pBuf.get());
}
void syncAllDevices() {
    while (1) {
        std::this_thread::sleep_for(
            std::chrono::seconds(hzi::config.timeSyncIntval));
        auto intvl = hzi::config.timeSyncIntval;
        for (auto& devicemap : hzi::config.devicesMap) {
            auto dev = devicemap.second;
            sync1(dev.id);
        }
    }
}
...
void syncWarningLeader(){
     while (1) {
        std::this_thread::sleep_for(
            ...
            auto flag = checkLoginInfo();
            if(flag == 1){
                string projectName = "";
                checkProName(projectName);
                string lead_msg = projectName + "该系统已经超过三天未有人员登录，请及时处理！";
                string cmd("./send_message.sh \"leader\" \"" + lead_msg + "\"");
                system(cmd.c_str());
...
void syncUpWSdata(){
    while(1)
    {
        ...
        auto startTimeAutoUp = ms_now - 30 * 60 * 1000;
        auto endTimeAutoUp = ms_now;

        //每周六8~10点上传基础信息（基础信息是 传感器坐标，每周上传一次）
        // if(isSaturdayMorning(ms_now,false))
        if(isSaturdayMorning(ms_now,true))
        {
            //是否 8--10点（周六）
            if(!isSaturday)
            {
                //发送基础信息
                // writeFTPFile(0,false,ms_now,ms_now);
                writeFTPFile(0,false,startTimeAutoUp,endTimeAutoUp,true);
                // cout<< "【"<< timestampToString(ms_now) <<"】:FTP上传一次基础信息.."<<endl;
                std::stringstream ss;
                ss<<"【"<< timestampToString(ms_now) <<"】:FTP上传一次基础信息..";
                logInfo(ss.str());
            }
            isSaturday = true;
        }
        ...
        //上传微震实际数据
        // writeFTPFile(1,false,ms_now,ms_now);
        writeFTPFile(1,true,startTimeAutoUp,endTimeAutoUp,true);
        // cout<< "【"<< timestampToString(ms_now) <<"】:FTP上传一次微震实际数据.."<<endl;
        std::stringstream ss;
        ss<<"【"<< timestampToString(ms_now) <<"】:FTP上传一次微震实际数据..";
        logInfo(ss.str());
        std::this_thread::sleep_for(std::chrono::minutes(30));
    }
}

void timer_handler_client(int signum)
{
    logInfo(string("Timer expired: 掘锚机通讯连接超时 "));
    hzi::isConnected = false ;
    string msg = " 掘锚机通讯连接超时 "; 
    // updateTunnelCurStatus(hzi::tunnel_mining_loc,hzi::heading_mil,hzi::isConnected,0,msg,true);
    updateTunnelLocInfo(hzi::tunnel_mining_loc, nowMS(), hzi::day_step, hzi::heading_mil, hzi::firstChnNO, true);
   
    auto ms = nowMS();
    hzi::endstamp = ms;
    hzi::isEndTime = false;
    hzi::serverStatus.lastSampTimes[8] = hzi::startstamp;
    insertTunnelDataInfo(hzi::startstamp,hzi::endstamp,0,hzi::start_tunnel_loc,
                            hzi::end_tunnel_loc,hzi::count_recv,hzi::isConnected);
        
    hzi::isStartTime = false;
    hzi::count_recv = 0;
...
void start_timer(){
    ...
    // 设置定时器的时间间隔
    client_timer.it_value.tv_sec = 20;
...
void syncMsevtsKafka()
{
    while(1)
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(hzi::config.kafkaSendInterval));//向卡夫卡发送消息的间隔
        auto ms_now = nowMS();
        
        uint64_t fromsampTime=ms_now -  hzi::config.kafkaSendInterval;//起点时间戳：当前时间-间隔
        uint64_t endsampTime=ms_now;
        std::cout << "kafka消息内容查询中" << std::endl;
        sendKafkaMsevtsMsg(fromsampTime, endsampTime);
   }
}
```
```cpp
//swt_surrock_process.hpp
#ifndef MS_TUNNEL_SURROUNDROCK_PROCESS_HPP
#define MS_TUNNEL_SURROUNDROCK_PROCESS_HPP
//随掘地震围岩评价处理
#include <memory>
#include <string>
#include <vector>
...
namespace ms_swt
{
    string SurroundRockDataProcess(MSTraceMgr& trcMgr, bool isRealtimeProc = false);
	string SurroundRockDataProcess(uint64_t dataTimestamp, bool isRealtimeProc = false);

    string SurRockDataProcBaseOnExtractInterfereSignal(MSTraceMgr& trcMgr);

}
#endif//MS_TUNNEL_SURROUNDROCK_PROCESS_HPP
```
```cpp
//swt_surrock_process.cpp
#include "swt_surrock_process.hpp"
#include "Config.hpp"
#include "common.hpp"
#include "ms_tracemanager.hpp"
#include "swt_common.hpp"
...
{
    //围岩评价参数
    struct SurrockProcParam
	{
		//基础参数
		bool isRealtimeProc = false;//是否实时处理
		bool doZero = true;			//零漂

		//滤波
		bool doFilter = false;
		int filterDomain = 0;//频域或时域
		float fltLoCutF = 60;//低截频
		float fltHiCutF = 300;//高截频
		float fltCutBand = 5;//截止带宽
		int fltConvLen = 301;//时域滤波因子长度
		int fltWndFlag = 2;	//窗函数标记
		bool fltIsStop = false;//是否带阻
		bool doNotch = false;	//是否陷波
		bool notchInFD=false;	//是否频域陷波
		float notchF = 50;	//陷波基频
		int notchOrder = 0;	//陷波阶数(整数倍、奇数倍、偶数倍、1倍)
		float notchBand = 5;	//陷波带宽
		int notchProcWay = 0;//陷波处理方式(置零、插值)

		//振幅均衡
		bool doDynamicEqualize = false;	//执行道内
		int dyequalWay = 1;				//道内均衡方法(AGC,指数增益)
		float dyequalTimeLen = 100;		//道内时窗ms
		float dyequalExpAlpha = 5;		//指数增益系数
		bool doTraceEqualize = false;	//执行道间
		int trcequalWay = 2;			//道间均衡方法(均值、最大值)

        //自干涉频谱
        bool doFreqSpctrmNormalize = false;  //是否归一化[0,1]
        bool doFreqSctrmTraceIntplt = false;//是否道插值
        float fsBegFreq = 60;       //起始频率
        float fsEndFreq = 500;      //结束频率
        int fsIntpltTraceNum = 4;   //插值道数
	};
     ...
     *
     */
    string GetSurrockProcessParam(SurrockProcParam &param, uint64_t datTimestmp)
    {
        string errmsg = "";

        //基础参数
		param.doZero = true;		 //零漂

        string sqlstr;
        auto conn = hzi::config.pConnPool->getConnection();

        // 1 信号处理参数
        if (param.isRealtimeProc)
        {
            sqlstr = "SELECT * FROM `e_mining_PCOSignalParm`";
        }
        else
        ...
        auto sqlRslt = conn.executeQuery(sqlstr.c_str());
        if (sqlRslt.next())
        {
            // 滤波
            param.filterDomain = sqlRslt.getInt("isCut");//频域或时域
            param.doFilter = (param.filterDomain>0);
            param.fltLoCutF = sqlRslt.getDouble("loCutF");//低截频
            param.fltHiCutF = sqlRslt.getDouble("hiCutF");//高截频
            param.fltCutBand = sqlRslt.getDouble("tranBW");//截止带宽
            param.fltConvLen = 301;//时域滤波因子长度
            param.fltWndFlag = sqlRslt.getInt("wndFlag");//窗函数标记
            param.fltIsStop = sqlRslt.getInt("isStop");//是否带阻（true 带阻滤波）
            param.doNotch = sqlRslt.getInt("isNotchCut");//是否陷波
            param.notchInFD = sqlRslt.getInt("notchInFD");//是否频域陷波
            param.notchF = atof(sqlRslt.getString("notchF"));//陷波基频
            param.notchBand = sqlRslt.getInt("NotchWn");//陷波带宽
            param.notchProcWay = sqlRslt.getInt("NotchType");//陷波处理方式(置零、插值)
            param.notchOrder = sqlRslt.getInt("Notch_jobs");//陷波阶数(整数倍、奇数倍、偶数倍、1倍)
            //振幅均衡	
            param.dyequalWay = sqlRslt.getInt("deAGCtype");//道内均衡方法(AGC,指数增益)
            param.doDynamicEqualize = param.dyequalWay > 0;//执行道内
            param.dyequalTimeLen = int(sqlRslt.getInt("wnLen"));//道内时窗ms
            param.dyequalExpAlpha = sqlRslt.getDouble("gainCoe");//指数增益系数
            param.trcequalWay = sqlRslt.getInt("interChnAmpEqualType");//道间均衡方法(均值、最大值)
            param.doTraceEqualize = param.trcequalWay > 0;//执行道间

            // 自干涉频谱（参数还未在表中建立）
            param.doFreqSpctrmNormalize = false;  // 是否归一化[0,1]
            param.doFreqSctrmTraceIntplt = false; // 是否道插值
            param.fsBegFreq = 60;                // 起始频率
            param.fsEndFreq = 500;               // 结束频率
            param.fsIntpltTraceNum = 4;          // 插值道数
        }
        else
        {
            errmsg = "无自干涉信号预处理参数";
    ...
    bool PreprocessSignalOneChn(MSDataTrace* trace, SurrockProcParam& procPara, int points, float deltT, int advnPoint)
    {
        try
        {
            thread_local float* pData = new float[points];

            memcpy(pData, trace->GetData(), sizeof(float) * points);
            if (procPara.doZero)
            {
                PureCommonOperation::DataCorrect(pData, points, PureCommonOperation::DCT_DEDC);
            }
            // 陷波
            if (procPara.doNotch)
            {
                if (procPara.notchInFD)
                {
                    PureCommonOperation::NotchFilter1dFD(pData, points, deltT, procPara.notchOrder, procPara.notchBand, procPara.notchProcWay, procPara.notchF);
                }
                else
                {
                    int maxOrder = static_cast<int>(std::floor(1000.0f / deltT / 2 / procPara.notchF));
                    TimeDomainNotch(pData, points, deltT, procPara.notchF, maxOrder);
                }
            }
            // 滤波
            if (procPara.doFilter)
            {
                PureCommonOperation::Filter1dFD(pData, points, deltT, procPara.fltLoCutF, procPara.fltHiCutF, procPara.fltCutBand, procPara.fltIsStop);
            }
            // 道内均衡
            if (procPara.doDynamicEqualize)
            {
                if (procPara.dyequalWay == 1) // AGC
                {
                    PureCommonOperation::DE_AGCGainCtrl(pData, points, int(procPara.dyequalTimeLen / deltT));
                }
                else if (procPara.dyequalWay == 2) // exp
                {
                    PureCommonOperation::DE_ExpGainCtrl(pData, points, advnPoint, procPara.dyequalExpAlpha, deltT / 1000);
                }
            }
            trace->SetData(pData, points);
            DELBFRS(pData);
            return true;
        }
        catch(const std::exception& e)
        {
            // std::cerr << e.what() << '\n';
            logErr(string("ProcessSignalOneChn:")+e.what());
            return false;
        }
    }
    ...
    {
        int chns = trcMgr.Head().traceCount;
        int points = trcMgr.Head().samplePoint;
        float deltT = trcMgr.Head().deltT;
        int advnPoint = trcMgr.Head().advanPoint;
        vector<MSDataTrace*>& vtTrace = trcMgr.GetTraces();
        float* pData = new float[points];

        //按道处理
        // for (int ch = 0; ch < chns; ch++)
        // {
        //     memcpy(pData,vtTrace[ch]->GetData(),sizeof(float)*points);
        //     if (procPara.doZero)
        //     {
        //         PureCommonOperation::DataCorrect(pData,points,PureCommonOperation::DCT_DEDC);
        //     }
        ...
        //         }
        //     }
        //     vtTrace[ch]->SetData(pData,points);
        // }
        // DELBFRS(pData)
        ...
        for (int ch = 0; ch < chns; ++ch)
        {
            std::packaged_task<decltype(PreprocessSignalOneChn)> preprocessSignalOneChnTask(PreprocessSignalOneChn);
            auto fut = preprocessSignalOneChnTask.get_future();
            futs.push_back(std::move(fut));
            ths.emplace_back(std::move(preprocessSignalOneChnTask), vtTrace[ch], std::ref(procPara), points, deltT, advnPoint);
        }
        for (auto& fut : futs)
        {
            if(!fut.get())
        ...
        //按炮处理
        if (procPara.doTraceEqualize)
        {
            TraceEqualize(vtTrace,points,procPara.trcequalWay,0,0);
        }
    ...
    //频谱计算
    bool CalFreqSpctrm(const MSTraceMgr& trcMgr, SurrockProcParam& procPara,DataFrame& dfData)
    {
        int points = trcMgr.Head().samplePoint;
        float deltT = trcMgr.Head().deltT;
        int chns = trcMgr.Head().traceCount;
        int fftPoint = PureCommonOperation::GetNextPower2(points);
        float deltF = 1000.0f / deltT / fftPoint;
        int freqBid = procPara.fsBegFreq / deltF;
        int freqEid = procPara.fsEndFreq / deltF;
        freqBid = freqBid<0?0:freqBid;
        freqEid = (freqEid>=(fftPoint/2-1)?(fftPoint/2-1):freqEid);
        freqBid = 0;//暂时和输入保持一致
        freqEid = points/2-1;
        ...
        int dfChns = chns;
        int dfPoints = points;
        if (procPara.doFreqSctrmTraceIntplt)
        {
            dfChns = (chns-1)*procPara.fsIntpltTraceNum+chns;
        }
        dfPoints = freqEid-freqBid+1;
        DataFrame df(dfChns,dfPoints);
        char *pHead = df.upHead.get();
        *(double *)(pHead) = (double)trcMgr.Head().sampleTime;
        *(uint8_t *)(pHead + 8) = AcorrItfe;
        *(float *)(pHead + 9) = deltF;
        *(uint32_t *)(pHead + 13) = dfChns;
        *(uint16_t *)(pHead + 17) = 1;
        *(uint32_t *)(pHead + 19) = dfPoints;
        *(uint32_t *)(pHead + 23) = 0;
        *(uint8_t *)(pHead + 27) = 1;

        //谱计算
        const vector<MSDataTrace*>& vtTrc = trcMgr.GetTraces();
        float* pRe = new float[fftPoint];
        float* pIm = new float[fftPoint];
        float* pOut = new float[dfPoints];
        float maxAmp = 0;
        int dfchnId = 0;
        for (int ch = 0; ch < chns; ch++)
        {
            memset(pRe,0,sizeof(float)*fftPoint);
            memset(pIm,0,sizeof(float)*fftPoint);
            memcpy(pRe,vtTrc[ch]->GetData(),points);
            PureCommonOperation::DataFFT(pRe,pIm,fftPoint,true);
            maxAmp = 0;
            for (int pt = 0; pt < dfPoints; pt++)
            {
                pOut[pt] = sqrtf(pRe[pt+freqBid]*pRe[pt+freqBid]+pIm[pt+freqBid]*pIm[pt+freqBid]);
                maxAmp = maxAmp<pOut[pt]?pOut[pt]:maxAmp;
            }
            if (procPara.doFreqSpctrmNormalize && maxAmp>0)
            {
                for (int pt = 0; pt < dfPoints; pt++) {
                    pOut[pt] /= maxAmp;
                }
            ...
            if (procPara.doFreqSctrmTraceIntplt) {
                dfchnId = ch*(procPara.fsIntpltTraceNum+1);
            }
            else {
                dfchnId = ch;
            }
            memcpy(df.upData.get()+dfchnId*sizeof(float)*dfPoints,pOut,sizeof(float)*dfPoints);
        }
        if (procPara.doFreqSctrmTraceIntplt)
        {
            int dfchnId2=0;
            float w1,w2;
            for (int ch = 0; ch < chns-1; ch++)
            {
                dfchnId = ch*(procPara.fsIntpltTraceNum+1);
                dfchnId2 = (ch+1)*(procPara.fsIntpltTraceNum+1);
                memcpy(pRe,df.upData.get()+dfchnId*sizeof(float)*dfPoints,sizeof(float)*dfPoints);
                memcpy(pIm,df.upData.get()+dfchnId2*sizeof(float)*dfPoints,sizeof(float)*dfPoints);
                for (int it = 0; it < procPara.fsIntpltTraceNum; it++) {
                    w2 = (it+1)*1.0f / (procPara.fsIntpltTraceNum+1);
                    w1 = (procPara.fsIntpltTraceNum-it)*1.0f / (procPara.fsIntpltTraceNum+1);                    
                    for (int pt = 0; pt < dfPoints; pt++) {
                        pOut[pt] = w1*pRe[pt]+w2*pIm[pt];
                    }
                    memcpy(df.upData.get()+(dfchnId+it+1)*sizeof(float)*dfPoints,pOut,sizeof(float)*dfPoints);
                }
            }
        }
        dfData = std::move(df);
    ...
    /**
     * @brief 保存频谱计算结果到文件11_fs，不入库
     */
    bool SaveAcorrIntfeSpctrm(uint64_t tmStamp, const string& filepath, const DataFrame& df)
    {
        bool ret = false;
        ...
        try
        {
            if (!fs::exists(filepathFs.parent_path()))
            {
                fs::create_directories(filepathFs.parent_path());
            }
            std::ofstream ofs_AcorrIntfeSpctrm(filepath, std::ofstream::binary);
            ofs_AcorrIntfeSpctrm.write(df.upHead.get(), 32);
            ofs_AcorrIntfeSpctrm.write(df.upData.get(), chns * points * 4);
            ofs_AcorrIntfeSpctrm.close();
        }
        catch (std::exception &e)
        {
            msg = "报错频谱文件失败";
            string errmsg = msg + filepath + ":" + e.what();
            if (hzi::config.verbose)
                cout << "errmsg = " <<errmsg << endl;
            debugErr(errmsg);
        }
    ...
    //围岩评价自干涉谱处理
    string SurroundRockDataProcess(MSTraceMgr& trcMgr, bool isRealtimeProc)
    {
        string retMsg = "";
        // 读取信号
        uint64_t timestampMs = trcMgr.Head().sampleTime;
        auto filepath = relLocFromTime(timestampMs) / std::to_string(SwtDataFrameType::AcorrItfe) / std::to_string(timestampMs);
        filepath = hzi::config.dataRoot / filepath;
        if (!fs::exists(filepath))
        {
            logErr(string("swt_surrock_process: file not exists"));
            return "无自干涉提取文件";
        }

        // 获取信号处理参数
        SurrockProcParam procPara;
        procPara.isRealtimeProc = isRealtimeProc;
        // GetSurrockProcessParam(procPara, timestampMs);
        
        // vector<ChnInfo> vtChnInfo;
        // errRet = GetProcessParam(timestampMs,procPara,imgPara,vtChnInfo);
        // if (!errRet.empty())
        // {
        //     return ("swt_advance_process:"+errRet);
        // }
        // if (vtChnInfo.size() != trcMgr.TraceCount())
        // {
        //     return ("swt_advance_process:trace count is mismatched!");
        // }
        // vector<MSDataTrace*>& vtTrc = trcMgr.GetTraces();
        // for (int i = 0; i < vtTrc.size(); i++)
        // {
        //     vtTrc[i]->SetShotRecvCoor(vtChnInfo[procPara.corrChnId].x,vtChnInfo[procPara.corrChnId].y,vtChnInfo[procPara.corrChnId].z,vtChnInfo[i].x,vtChnInfo[i].y,vtChnInfo[i].z);
        //     vtTrc[i]->Head().status = vtChnInfo[i].enb;
        // }
        
        // 预处理信号
        if (!PreprocessSignal(trcMgr,procPara))
        {
            logErr(string("swt_surrock_process:signal process abort"));
            return ("自干涉信号预处理错误");
        }
        ...
        //保存结果记录
        string filepathFS = filepath.string() + "_fs";
        retMsg = ms_swt::SaveDataFrameToFile(filepathFS,dfData);
        return retMsg;
    }
    string SurroundRockDataProcess(uint64_t timestampMs, bool isRealtimeProc)
    {
        string retMsg = "";
        MSTraceMgr trcMgr;
        ...
        if (!ReadFromDataFrameFile(filepath,trcMgr))
        {
            logErr(string("swt_surrock_process:read interference data file failed!"));
            return "读取自干涉提取文件错误";
        }
        ...
        SurrockProcParam procPara;
        procPara.isRealtimeProc = isRealtimeProc;
        retMsg = GetSurrockProcessParam(procPara, timestampMs);
        if(!retMsg.empty())
            return retMsg;
        ...
        string filepathFS = filepath.string() + "_fs";
        retMsg = ms_swt::SaveDataFrameToFile(filepathFS,dfData);
        // SaveAcorrIntfeSpctrm(timestampMs, ,dfData);

        return retMsg;
    }

     /**
     * @brief 基于自干涉提取信号进行频谱计算（未进行信号处理)
     */
    string SurRockDataProcBaseOnExtractInterfereSignal(MSTraceMgr& trcMgr)
    {
        string retMsg = "";
        ...
        // 获取信号处理参数
        SurrockProcParam surRockProcPara;
        surRockProcPara.isRealtimeProc=false;
        retMsg = GetSurrockProcessParam(surRockProcPara,timestampMs);
        if(!retMsg.empty())
            return retMsg;
        surRockProcPara.doZero=true;
        surRockProcPara.doNotch=false;
        surRockProcPara.doFilter=false;
        surRockProcPara.doDynamicEqualize=false;

        //3 处理信号
        if (!PreprocessSignal(trcMgr,surRockProcPara))
        {
            logErr(string("swt_surrock_process:signal process abort"));
            return ("自干涉信号预处理错误");
        }
        ...
        //4 计算频谱
        DataFrame dfData;
        CalFreqSpctrm(trcMgr,surRockProcPara,dfData);
...
void realTimeCal() 
{
  ...
  while (1) 
  {
    ...
    if (it != hzi::fromToEndMap.end()) 
    {
      ...
      if (diff <= 0)
      {
        logErr(to_string(fromStamp) + " <= " + to_string(endStamp) + ", error..");
        return;
      }
      ...
      if (rslt0.next()) 
      {
          ...
          hzi::miningDateLen = rslt0.getInt("dataLen");//数据时长（单位时间min)
          hzi::mining_classId = rslt0.getInt("class_Id");//拼接数据类型（6 微震全时 3-微震背景）
          logInfo("realtime process data class_id=" + to_string(hzi::mining_classId));
      }
    ...
    {
      logInfo(string("realtime process no data: hzi::fromToEndMap is empty.."));
    }
  }
}
```
