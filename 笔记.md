...
namespace {
    std::map<int, size_t> mapbytesSum;
...
tl::expected<int, string> handleRecvCmd(uint8_t cmdId, uint32_t cmdSeq,
    ...
    switch (cmdId) {
        case 1:
            return recvBaseStationCheck(cmdSeq, std::move(pData));
            break;
        case 2:
            return recvClockCalibr(cmdSeq, devAddr, std::move(pData));
            break;
        case 3:
            return recvConfig(cmdSeq, devAddr, std::move(pData));
            break;
        case 4:
            return recvData(cmdSeq, devAddr, std::move(pData));
        case 5:
        case 6:
        case 7:
        case 8:
...
void receive_frame(std::unique_ptr<char[]> buf, size_t bytesGot) {
    try {
        ...
        if (cmd_id == 8) {
            ...
            sync2(addr, microSecs);
            return;
        }

        int32_t cmd_seq = be32toh(*(int32_t *)(pBuf + 12)); // 命令标识

        size_t dataLen = bytesGot - 22;
        ...
        if (hzi::config.verbose && cmd_id != 9) {
            ...
            if (cmd_id == 4) {
                ...
                cout << ",采样时间:" << be64toh(*(int64_t *)(pBuf + 16));
        ...
        if (tail != 0x1616) {
            ...
            if (cmd_id == 4) {
                ss << ",数据类型:" << (unsigned)*(uint8_t *)(pBuf + 24)
                    << ",数据区字节数:" << dataLen;
            }
...
void handle_buf(int port, size_t dataBytes, std::unique_ptr<char[]> &buf1,
    ...
    if (left >= 0) {
        ...
        std::thread handler(receive_frame, std::move(buf1), succs + 8);
        if (handler.joinable()) {
            handler.detach();
        }
        ...
        // buf1放空
        buf1 = std::make_unique<char[]>(hzi::config.recvBufsize);
        if (!buf1) {
            logErr("接收缓冲区分配内存出"
                   "错，退出");
            exit(1);
...
std::unordered_map<int, std::unique_ptr<char[]>> mapBufA;
std::unordered_map<int, std::unique_ptr<char[]>> mapBufB;

std::unordered_map<int, char *> map_pBufA;
...
void startReceiver2(int port) {
    ...
    try {
        ...
        Server server = std::move(serverExp.value());
        string msg("端口");
