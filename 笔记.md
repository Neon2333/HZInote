...
> * 自动更新：
>
>   【掘进里程】每天自动增加，此时传感器若未挪动，会导致【偏移距】也跟着更新增大。（当每日进尺和实际一样时，保持自动增加就行。当实际每日进尺和设定不一致时就需要手动修改掘进里程）。
>
> * 手动修改：
>
>   【掘进里程】：当实际每日进尺和设定不一致时需要手动修正掘进里程，并更新平均日进尺。若传感器未挪动，偏移距也跟着变化，计算出新的偏移距。
>
>   【偏移距】不会每天改，只有当偏移距过大的时候，会挪动一组传感器到距离迎头一定位置处，然后改小偏移距。这时偏移距就是新的传感器距离迎头距离了，需用户手动修改【偏移距】，更新传感器坐标。
>
...
* 问题：点击【修正】修改掘进里程后，若不点击【修改-更新】就刷新的话，会导致偏移距意外变化（随着掘进里程改变）。
  避免：保证每次点击【修正】后，都立即点击【修改】更新。中间不刷新。

  改进想法：修改这几个按钮的逻辑，点击【修正】后会拿【修改】中的当前偏移距和基准通道更新一次，从而避免用户在点击【更新】前，做一些其他的操作（如：刷新页面）。
...
| -                     | -                                                            |
| --------------------- | ------------------------------------------------------------ |
| p_chn_types           | 通道type_id和类型                                            |
| e_chns_config         | 通道配置显示的表。保存用户输入的通道坐标。有update触发器，更新到`e_chns_config_history`。 |
| e_chns_config_history | 保存`e_chns_config`修改历史                                  |
| e_chns_config_tunnel  | 修改偏移距后，先根据delta，修改该表中通道坐标。再更新到`e_chns_config` |
|                       |                                                              |
| e_tunnel_cur_status   | 基准通道，偏移距，掘进里程。前端发送的掘进里程和偏移距，存到全局变量后，直接入库该表。根据日进尺的自动计算也入库该表。 |
| e_mining_PCOLocParm   | 点击【当前掘进里程，修正】，只保存该按钮点击后前端发来的：掘进方向（0-x正向，1-负向），日进尺，掘进里程，修改参数的时间戳 |
| e_tunnel_locInfo      | 时间戳，日进尺，掘进里程                                     |
|                       |                                                              |
|                       |                                                              |
...
>
> 调用`updateTunnelCurStatus`更新掘进里程和偏移距到表`e_tunnel_cur_status`。
>
> 根据【掘进里程】计算采空率。
...
* 每天【掘进里程】和【偏移距】自动增加：
  ...
  if (rslt.next())
  {
      ...
      // 根据时间戳在当日时刻，在细分进尺量
      mining_locx = (m_dir == 0) ? (mining_locx + days * dayStep)
          : (mining_locx - days * dayStep);
      hzi::heading_mil = mining_locx - hzi::firstChn_locx;//计算偏移距
      auto ret1 = updateTunnelCurStatus(
          mining_locx, hzi::heading_mil, hzi::isConnected,
          hzi::isRunning, "更新掘进位置", false);
  }
  ```
...
> * `time_t`或`uint_64`格式变量在`execute()`中执行时，都要先转double
> * 
...
库表字段数据类型设为`datetime`，4位

调用函数`FROM_UNIXTIME()`将unix时间戳（单位s）转成`datetime`插入

时间戳类型`time_t`或`uint64_t`需要转成double才能被`libzdb`库的函数`execute`使用

datetime类型字段用`conn.getDateTime("field")`从查询结果中获取值
...
"values(valNum) , vecSize= values(vecSize),outLen= values(outLen)",
(uint8_t)devId,  // dev_id
(double)tmStmp / 1000.0, // tmStmp是uint64_t因此要转double
classId2,        // class_id
deltT,           // curt_ch
valNum,          // channels
vecSize,
