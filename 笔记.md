...
>
> > 若用【背景】原始数据计算，提取数据时间戳为时间段**开始时间加上5s**。
> >
...
`/handerTunnelSignal/:start_loc/:end_loc/:time_len`，调用回调`handerSignalProcess`。

调用`mergeMs`拼帧，将1炮数据拼成一个`DataFrame`，所有炮存在` std::vector<DataFrame *> dfvec`。

`handleSignalProcess`对`dfvec`中数据进行处理。
...
if (dfvec.size() > 0) {
    ...
    logInfo("start handleSignalProcess");
    string filepath = handleSignalProcess(dfvec, false,dev_id);
    string rslt("{");
...
![image-20250429112255402](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291122554022)

**下面的计算都是针对1炮，某个dex的数据的处理。**

##### 加窗+预处理

![image-20250429111239334](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291112393341)
...
* 取通道号，以及该通道号对应的因子道号，将因子道数据暂存到`coefData`。
  ...
  ![image-20250429140811344](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291408113443)
...
![image-20250429141332632](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291413326325)

![image-20250429141212378](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291412123784)

![image-20250429141437245](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291414372455)

![image-20250429142151631](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291421516316)

##### 相关计算

#### 一维滤波

![image-20250429143704862](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291437048626)
...
![image-20250429155438319](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291554383197)

若启用滤波，则反褶积后，会再次进行滤波，且写死为带通滤波。

#### 道内均衡

![image-20250429155722965](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291557229657)

#### 道间均衡

![image-20250429155753889](C:\Users\Admin\AppData\Roaming\Typora\typora-user-images\image-202504291557538898)
...
```cpp
        ...
        *(double *)(pHead2) = (double)tmStmp;//时间戳存32位头
        // 19 代表随采，9是随掘
        auto classId2 = 9;
        ...
        *(uint8_t *)(pHead2 + 27) = devId;              // 基站ID
		...
		//入库表e_mining_signaldata_frm_info
        conn21.execute(
            "INSERT INTO "
		...
		try
        {
            ...
            for (int dex = 0; dex < vecSize; dex++)
            {
                ...
                for (int ch = 0; ch < valChnns; ++ch)
                {
                    ...
                    //每个通道的数据大小：点数*4字节，每个通道的点数：pcoParm.outLen
...
```cpp
// fg:是否是实时计算，历史计算异常不提取结果 typeId用于分量类型 0-X 1-Y 2-Z
                ...
                bool fg, bool outType)
{
    ...
    { 
        ...
        // float* pOut_2 = new float[iGrdNum];  // 异常提取
        // memset(pOut_2, 0, sizeof(float) * iGrdNum);
    ...
    // float* pOut_0 = new float[iGrdNum_out];
    // memset(pOut_0, 0, sizeof(float) * iGrdNum_out);
    ...
    *(int32_t *)(pHead + 24) = *((int32_t *)&dX);
    ...
    auto classId2 = (typeId == 0) ? 10 : (100 + typeId);
