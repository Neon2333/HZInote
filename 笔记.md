...
| 1                             | 激电                                                         |
| 2                             | 微震事件                                                     |
| 3                             | 微震背景（采样间隔2ms，每个通道2500点），存储的是极值（摘要文件digest_.dat中单位时间内的极大值或极小值） |
| 4                             | 温度数据                                                     |
| 5                             | 耦合数据                                                     |
...
| 0          | 8          | 采样时间                                                     | double   | **数据帧内第一个采样数据点的时间戳**，毫秒。（每个时间点的精确采样时间戳可由`该值+i*采样间隔`计算） |
...
> [!CAUTION]
>
> * 拼帧mergeMS()根据`hzi::mining_classId`选择原始数据类型。根据`samp_time>fromtime`而不是`>=`挑选原始数据的时间戳。使得拼得到的数据帧的`sampTime()`获取的时间`+2.5s`或`+5s`。从而解决全时/背景提取数据都写入表`e_mining_signaldata_frm_info`中主键`samp_time`冲突的问题。
...
/handerExtSignalParm/:samp_time/:typeId——handerExtSignalParm()
...
> |         |                        |                                                              |
> | 9       | 干涉提取数据文件       |                                                              |
> | 99      | 随掘——互干涉再提取文件 | 9文件经过信号处理流程后的再提取数据文件。注意：该文件classid=9，而不是99 |
> | 10      | 偏移图数据文件         |                                                              |
> |         |                        |                                                              |
> |         | 随掘——自干涉提取文件   |                                                              |
> |         |                        |                                                              |
...
| **9**   | 4       | **float**    | **频率分辨率frequencyResolution：1000.0 / (deltT*pcoParm.outLen);** |
|         |         |              |                                                              |
