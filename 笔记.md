...
#### 历史重算接口调用回调`handerTunnelSignalProcess`
...
根据传入的时间段，从表`e_warning_Info`查询已有的预警信息，进行删除。从表`e_msevt_rslts`中重新查询该时间段的微震事件，进行预警的逻辑计算，得到新的预警记录入库表`e_warning_Info`。（工程人员会修改表`e_msevt_rslts`中的微震记录，之前的记录是系统自动判别的。）
