...
> [!IMPORTANT]
>
> 实时：自动进行【提取+再提取+偏移】，保存提取文件9，保存偏移文件10。
>
...
>   | `setSignalChnInfo`读取参数到`pcoExtractChnParms`             |      |
>
...
/*
	* 陷波
	* Notch_jobs ：陷波时剔除50hz的奇偶倍数：0-50hz的整数倍，1-奇数倍 2-偶数倍
...
>   /data_file/:year/:month/:day/:type/:timestamp  
>   获取99文件（如果有的话：也就是这个时间已经点击过执行做过再提取的话，就会有99。若是第一次进行历史重算就没有，会返回204 No Content）
>       
>       
...
> * 点击【执行】
>
>   显示【提取图+再提取图+偏移图（提取+再提取上的偏移图）】
>
>   使用左侧设定参数，在提取文件的基础上，进行再提取。根据设定好的计算参数对当前选中时间戳的提取数据，进行滤波/振幅均衡/反褶积/反射波提取等操作。生成处理后的`再提取数据(class_id=99)`和`再提取波形图`，并在再提取数据和再提取波形图的基础上，得到新的偏移图（提取+再提取的偏移图）。
>
>   会调用接口：
>
>   ```cpp
