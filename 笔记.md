...
查询表`e_mining_pcodata_frm_info`获取数据时间戳。
...
```json
{
    ...
    //"mysql_url": "mysql://127.0.0.1:3312/semtdb1?user=hzidba&password=dzaqZHk3",//开发服务器
...
* 200成功
* 405接口的方法类型搞错了。比如一个接口是POST方法，发了GET请求。
...
| /table/:tblName/:delCondition?                           | 插入记录或删除记录                                           |          |
|                                                          |                                                              |          |
