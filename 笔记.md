...
//sys_config.kwang.json
{
    ...
    "link_device": false,//hzi::config.linkDevice
    
    "time_sync": false,//hzi::config.timeSync
    ...
    "ms_grid_step": 0.1,
    "ms_mining": true, //是否随采
    "ms_mining_type": 2,//hzi::config.ms_mining_type，随采类型：0-无随采，1-随采，2-随掘
    "ms_miningSignal": true,//
    "save_full_msdata": false,//是否保存全时微震数据帧（入库+写文件）
    "ms_trigger":true,//是否保存微震事件
    "ms_tunnel": true,//
    "warningType": 1,//warningType 异常预警计算类型（约定 1：铁建移动检波器时异常聚类  2：根据时间进行聚类提取 3：样本数量 4：提取次数）
    "ms_interface_type": 1,//采集页面维度类型 0-XY 1-XZ 2-XYZ
    "digest_duration": 5,
    ...
    // "alarm_receivers":"GuoJinShuai",//预警用户
...
| msChannelsLabels                                             |                          | 地震通道的label，形如：S1GR01                                |
...
>
> * is_saveFile()——根据【数据存储策略】判定是否保存数据帧
>
> * handleData()——数据帧
>
>   > * DataFrame::save()
>   >
>   >   > * DataFrame::saveIntoDataFrmInfo()——根据配置文件字段将数据帧信息入库表
>   >   >
>   >   > * **DataFrame::saveIntoDataDigest()**——保存摘要（全时微震，温度，伽马，激电数据帧不保存摘要）信息入库表
>   >   >
...
>   >
>   > * createEngDensity()——数据帧中数据写入【平均能量文件】
>   >
>   >   > * setupDensityFile()——创建摘要文件，并通过mio库映射到内存
>   >
>   > * handleTimeStamp()——
>   >
>   >   > * getEngDenityParm()
...
void start_http_server() {
    ...
    app.init(opts);
    app.useSSL("etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.pem",
               "etc/cert/ahhzi.com.cn/6803107__ahhzi.com.cn.key");//useSSL 方法用于配置服务器的 SSL 证书和密钥文件路径，启用 HTTPS 支持。
...
const char* ResultSet::getString(int columnIndex);
...
| 16         | 1          | 激电发射帧序号或微震触发通道号（用于微震的信号触发时值最大的通道） | uint8_t  |                        |
