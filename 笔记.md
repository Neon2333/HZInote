...
| 28      | 1     | uint8_t      | sysType                                                      |
| 29      | 1     | uint8_t      | imageType                                                    |
...
> [!IMPORTANT]
>
> * 11代表自干涉数据大类：文件名时间戳——提取数据，文件名时间戳_fs——频谱数据
>
> * 自干涉文件路径入库表`e_mining_selfcorrdata_frm_info`
>
> * 自干涉谱图文件，文件名以`时间戳_fs`，不入库（同样放于11目录）。
>
...
> * 后端落实
>
>   > * 新增字段，加到库表`e_mining_PCOSignalParm`
>   >
>   > * 在接口函数加自相关频谱计算函数`selfCorr`
>   >
>   >   ```cpp
>   >   /handerSignal/:from_time/:to_time/:time_len/:devId（）
>   >   handerSignalProcess();	//加入自相关提取过程
>   >   ```
>   >
>   > 
